...
From a value of accuracy of around 0.802, we can say that our K-nearest neighbour classifier predicted the correct class 
...
With five different shuffles of the data, we get five different values for accuracy!
...
classifier built using our overall training data. We can combine the estimates by taking their average to
...
around 81%. Whether 81% is good or not depends entirely on the downstream application of the data analysis. In the 
present situation, we are trying to predict a tumour diagnosis, with expensive, damaging chemo/radiation therapy
or patient death as potential consequences of misprediction. Hence, we'd like to do better than 81% for this application. 
...
train_control <- trainControl(method="cv", number = 10)
k = data.frame(k = seq(from = 1, to = 40, by = 1))
...
Although `caret` provides a selection of $K=26$ for us by maximizing the accuracy estimate, it is helpful regardless to visualize
the accuracy as we increase $K$. We can access the results from the cross-validation via the`results` attribute of 
the `train` object (our classifier). We use this to plot accuracy versus $K$:

```{r 06-find-k, fig.height = 4, fig.width = 5}
accuracy_vs_k <- ggplot(knn_model_cv_10fold$results, aes(x = k, y = Accuracy)) +
  geom_point() +
  geom_line()
accuracy_vs_k
```

Based off of the visualization above, we might agree with `caret`'s choice of $K=26$. But as you can see, there is no exact or perfect 
answer here; any choice between $K=20$ and $K = 30$ would be reasonably justified. Remember: the values you see on this plot are *estimates*
of the true accuracy of our classifier. Although the $K=26$ value is higher than the others on this plot, that doesn't mean the classifier is
...
- changing the value to a nearby one (e.g. from $K=26$ to 25 or 27) doesn't decrease accuracy too much, so that our choice is reliable in the presence of uncertainty
...
train_control <- trainControl(method="cv", number = 10)
k_lots = data.frame(k = seq(from = 1, to = 300, by = 5))
set.seed(1234)
...
You can see this effect in the plots below as we vary the number of neighbours $K$ in (1, 26, 200, 300):
...
ks <- c(1, 26, 200, 300)
plots <- list()
scaled_train_total <- X_train_total %>%
                         mutate(Class = Y_train_total)
...
for (i in 1:length(ks)){
	...
	con_grid <- seq(min(scaled_train_total$Concavity), max(scaled_train_total$Concavity), length.out = 100)
	smo_grid <- seq(min(scaled_train_total$Smoothness), max(scaled_train_total$Smoothness), length.out = 100)
	...
	plots[[i]] <- ggplot() + 
	    ...
	    geom_point(data = scaled_train_total, mapping=aes(x = Concavity, y = Smoothness, color = Class), alpha=0.75) +
	    labs(color = "Diagnosis") + 
            ggtitle(paste('K = ', k)) +
	    scale_color_manual(labels = c("Benign", "Malignant"), values = cbPalette)
}

...
as meaningful predictors are added, the estimated accuracy increases; and as you add irrelevant
variables, the accuracy either exhibits small fluctuations or decreases as the model attempts to tune the number
of neighbours to account for the extra noise. Given that generally speaking, you should prefer a simpler model (it has a 
lower variance and less chance of overfitting), the right trade-off of accuracy and number of predictors
occurs with the model `Class ~ Perimeter + Concavity + Smoothness`. We have successfully removed irrelevant
...
whether adding a variable provides a meaningful increase in accuracy.

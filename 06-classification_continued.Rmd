...
perim_concav <- cancer |>
  ...
  scale_color_manual(labels = c("Malignant", "Benign"), values = c("#E69F00", "#56B4E9"))
...
then we would hope that roughly 65% of the training data are benign, 35% are malignant,
...
> Note: We use the `glimpse` function to
> examine the data because it has a large number of columns; `glimpse` outputs the data 
> such that the columns go down the page (instead of across), which allows us to see 
...
value of another, categorical variable. The K-nearest neighbour algorithm in

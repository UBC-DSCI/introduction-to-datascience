...
that
we created ourselves using a random number generator.
You will see from the scatter plot below with two example
irrelevant variables `Irrelevant1`, `Irrelevant2` that
...
but also increasingly many irrelevant variables (from 0 to 500 of them). Each model is tuned via 5-fold cross-validation.
...
do we choose which variables we *should* use?  
A simple&mdash;but often very effective&mdash;method is to rely on your scientific understanding
...
as a predictor. On the other hand, the remaining variables all seem like reasonable candidates; it 
is not clear which subset of them will create the best classifier.

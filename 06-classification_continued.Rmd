...
1. Simple, intuitive algorithm
2. Few assumptions about what the data must look like 
3. Works for binary (two-class) and multi-class (> 2 classes) classification problems

**Weaknesses:**

1. As the training data gets larger, K-nearest neighbours becomes very slow
...
variables from your data will be treated as predictor variables. Based
on your initial exploratory analysis, you can choose
...
as well as 500 irrelevant variables
named `Irrelevant1` to `Irrelevant500`, which
we generated ourselves using a random number generator.
You will see from the scatter plot below with two example
variables `Irrelevant1`, `Irrelevant2` that
these irrelevant variables have no meaningful relationship
...
Next, we build a sequence of K-NN classifiers that include both `Smoothness` and `Concavity` as predictor variables,
but also increasingly many irrelevant variables. Each model is tuned via 5-fold cross-validation.
We show the plot of estimated accuracy versus the number of irrelevant predictors below.
<center>
...
Wow! Once we add sufficiently many irrelevant predictor variables, our classifier
...
So then if we can't just throw in the kitchen sink, how do we choose which
variables to use as predictors?  This is a very difficult question to answer in

...
To perform 5-fold cross-validation in R with `tidymodels`, we
...
data frame with the `neighbors` variable containing each value from $K=1$ to $K=15$.
...
training over 1000 models with $m=10$ predictors, forward selection requires training only 55 candidate models.

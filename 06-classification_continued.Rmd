...
covered training and data preprocessing, this chapter focuses on how to evaluate prediction accuracy and how to 
choose model parameters to improve it. 
...
- Perform K-nearest neighbour classification in R using `caret::train(method = "knn", ...)` to predict the class of a test dataset
...
if it provides accurate predictions on data *not seen during training*.

But how can

 One way to assess the performance of a classification algorithm is to split the data into a **training set** and 
a **validation set**. The **training set** is used to build the classifier.

 Then we can give the observations 
from the **validation set** (without the labels/classes) to our classifier and predict the labels/classes as 
if these were new observations that we didn't have the labels/classes for. Then we can see how well our predictions 
match the true labels/classes for the observations in the **validation set**. If our predictions match the true 
labels/classes for the observations in the **validation set** very well then we have some confidence that our 

...
So if we can use more than two predictor variables in K-nearest neighbour classification,
shouldn't we always just use *all* of our variables? Unfortunately, K-nearest neighbours
does not necessarily the perf more variables it is not quite that simple.


We were quite fortunate in our initial exploration to find two variables (perimeter and concavity)
that, together, seem to separate benign and malignant observations into two distinct regions.
But what should we do if we cannot immediately find such a nice collection of variables?
Well, sometimes it is just a fact that the variables in the data do not have enough of 
a relationship with the response variable to provide accurate predictions. For example,
in the breast cancer image data, if the only available predictor was "patient's favourite icecream flavour",
we likely would have little hope of using that variable to predict the response 
(barring any future incredible scientific discoveries about the relationship between icecream preference
and cancer diagnosis). In cases like these, the only option is to go and measure more useful variables.

There are, however, a wide variety of cases where the predictor variables *do* have a 
meaningful relationship with the response variable, but that relationship is not obvious
if you just plot the variables directly. In these cases, you will need to use some of the wrangling
tools from previous chapters to manipulate the variables into a useful form, which is sometimes
...
- [An Introduction to Statistical Learning](https://www.statlearning.com/) [-@james2013introduction] provides a great next stop in the process of learning about classification. Chapter 4 discusses additional basic techniques for classification that we do not cover, such as logistic regression, linear discriminant analysis, and naive Bayes. Chapter 5 goes into much more detail about cross-validation. Chapters 8 and 9 cover decision trees and support vector machines, two very popular but more advanced classification methods. Note that while this book is still a very accessible introductory text, it requires a bit more mathematical background than we require.

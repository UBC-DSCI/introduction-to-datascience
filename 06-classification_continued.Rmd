...
You can see this effect in the plots below as we vary the number of neighbours $K$ in (1, 7, 50, 100):

<center>
```{r 06-decision-grid-K, echo=FALSE, fig.height = 4, fig.width = 5}
library(repr)
library(gridExtra)
...
for (i in 1:length(ks)){
	...
	plots[[i]] <- ggplot() + 
	    geom_point(data = prediction_table, mapping=aes(x = Concavity, y = Smoothness, color = Class), alpha=0.02, size=5.)+
...
**(TODO fix this plot; they are way too small...)**
...
accuracy. To tune the classifier (e.g., select the K in K-nearest neighbours), we compute and only using the classification
...
The overall workflow for performing K-nearest neighbour
classification in `caret` is as follows:

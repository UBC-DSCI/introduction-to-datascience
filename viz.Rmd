...
In general, the guiding principles of when to use what type of plot, 
for those introduced in this book (Figure \@ref(fig:plot-sketches)), 
are as follows:
...
data points have a natural, or inherent order. 
Specifically, each observation in the data set has a predecessor 
and a successor.
In such a case, the order of the observations matter&mdash;changing their order 
will alter the meaning of the data.
Given this, we typically use a line plot 
to connect the observations when we are visualizing time.
This helps illustrate the temporal order of the observations.

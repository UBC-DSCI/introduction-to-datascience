...
```{r plot-sketches, echo = FALSE, fig.width = 4.6, fig.height = 4.75, fig.align = 'center', fig.cap = "Examples of a scatter, line and bar plots, as well as histograms."}
set.seed(1)
...
because it helps space out small values and squish larger values together.
...
Figure \@ref(fig:03-mother-tongue-vs-most-at-home-scale) is the appropriate
visualization to use to answer the first question in this section, i.e.,
whether there is a relationship between the number of people who speak 
a language as their mother tongue and the number of people who speak 
that language as their primary spoken language at home.
...
In Figure \@ref(fig:03-mother-tongue-vs-most-at-home-scale), we see that 
...
Furthermore, because the points in Figure \@ref(fig:03-mother-tongue-vs-most-at-home-scale) 
...
In Figure \@ref(fig:03-scatter-color-by-category), the points are colored with
...
even gathering additional data. We will see more of such complex analyses in later on in 
this book.  
...
and continents are colored differently than other landmasses, making it quite clear that continents are the largest seven landmasses.
...
(1) ensuring a color is mapped as discretely where appropriate (like in this example) and (2) the ordering of levels in a plot. `ggplot` takes into account the order of the factor levels as opposed to the order that is displayed in your data frame. Learning how to reorder your factor levels will help you with reordering the labels of a factor on a plot.  
 
Unfortunately, the attempt to separate out the experiment number visually has created a bit of a mess. All of the colors in Figure \@ref(fig:03-data-morley-hist-3)
...
In this case, we can see that in this case, both the default number of bins 
...
using the assignment symbol (`<-`), we can call it 
and add to it using the `+` operator.
...
the Mauna Loa observatory from Hawaii, consisting of CO$_{\text{2}}$ measurements from 1980 to 2020. We plotted the measured concentration of CO$_{\text{2}}$ (on the vertical axis) over time
...
**Bitmap / Raster** images are represented as a 2-D grid of square pixels, each with its own color. Raster images are often *compressed* before storing so they take up less space. A compressed format is *lossy* if the image cannot be perfectly recreated when loading and displaying, with the hope that the change is not noticeable. *Lossless* formats, on the other hand, allow a perfect display of the original image.

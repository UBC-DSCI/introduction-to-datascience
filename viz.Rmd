...
Tables are another medium used to answer to questions with data 
and communicate the answers.
However, visualizations are usually more effective 
at communicating the big picture, take home message. 
For this reason, data visualizations are widely used 
and the focus of this chapter.
However, when the goal of communication is precision and details, 
tables can then be a better choice.
...
and is of type `date`.
The `ppm` columns holds the value of $CO_2$ in parts per million 
...
Let's now try tp visualize the `co2_df` as a line plot 
...
on just five years of data (say, 1990-1995).
...
```{r 03-data-co2-line-3, warning = FALSE, message = FALSE, fig.cap = "Line plot of atmospheric concentration of CO$_{2}$ from 1990 to 1995 only"}
library(lubridate)
...
co2_line <- ggplot(co2_df, aes(x = date_measured, y = ppm)) +
  ...
  xlab("Year") +
  ylab("Atmospheric CO2 (ppm)") +
  xlim(c(date("1990-01-01"), date("1994-12-01"))) +
  theme(text = element_text(size = 16))
co2_line
```
...
A bar plot uses a rectangle to encode the value a summary statistic 
...
bar_data <- tibble(count = c(35, 27, 21),
                   category = as.factor(c("Group 1", "Group 2", "Group 3")))
...
The first argument passed to `fct_reorder` is the factor columns's name, 
whose level's we would like to reorder (here `landmass`). 
...
We do this here so that the largest bar will closet to the axis line,
as that is more visually appealing.
...
however `landmass_type` is not very human readable or visually pleasing.
...
under the name `faithful` so it does not need to be loaded.
...
to show how many data points fell in each bin. 
...
unless we can see what the true value is.
...
Here we set it to the true value of light speed: 792.458. 
We also would like to fine tune this vertical line, 
styling it so that is dashed and 1 point in thickness. 
...
```{r 03-data-morley-hist-binwidth, echo = FALSE, warning = FALSE, message = FALSE, fig.height = 10, fig.cap = "Histogram of relative accuracy split vertically by experiment with clearer axes and labels"}
morley_hist_default <- ggplot(morley_rel, aes(x = relative_accuracy, fill = factor(Expt))) +
  geom_histogram(position = "identity") +
  facet_grid(rows = vars(Expt)) +
  geom_vline(xintercept = 0, linetype = "dashed", size = 1.0) +
  labs(x = "Relative Accuracy (%)", y = "# Measurements", fill = "Experiment ID") +
...
morley_hist_big <- ggplot(morley_rel, aes(x = relative_accuracy, fill = factor(Expt))) +
...
morley_hist_med <- ggplot(morley_rel, aes(x = relative_accuracy, fill = factor(Expt))) +
...
morley_hist_small <- ggplot(morley_rel, aes(x = relative_accuracy, fill = factor(Expt))) +

...
we can use a K-nearest neighbours-based \index{K-nearest neighbors!regression} 
approach in regression to make predictions. 
Let's take a small sample of the data in Figure \@ref(fig:07-edaRegr) 
and walk through how K-nearest neighbours (K-NN) works
...
we find and label the 5 nearest neighbours to our observation 
...
the model and returns the RMSPE for each number of neighbours. In the output of the `sacr_results`

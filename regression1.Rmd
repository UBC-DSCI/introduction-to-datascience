...
use `tidymodels` workflows, we will use a K-nearest neighbours (KNN) 
...
* Explain the K-nearest neighbour (KNN) regression algorithm and describe how it differs from KNN classification.
...
* Evaluate KNN regression prediction accuracy in R using a test data set and an appropriate metric (*e.g.*, root means square prediction error).
...
neighbours-based \index{K-nearest neighbors!regression} approach in regression to make predictions. Let's take a small
sample of the data above and walk through how K-nearest neighbours (K-NN) works
...
neighbours to our observation of a house that is 2,000 square feet.
...
```{r 07-knn3-example, echo = FALSE, fig.height = 4, fig.width = 5, fig.cap = "Scatter plot of price (USD) versus house size (square feet) with lines to 5 nearest neighbours"}
...
```{r 07-predictedViz-knn, echo = FALSE, fig.height = 4, fig.width = 5, fig.cap = "Scatter plot of price (USD) versus house size (square feet) with predicted price for a 2,000 square-foot house based on 5 nearest neighbours represented as a red dot"}
nn_plot +
...
This stems from the use of nearest neighbours to predict values.
...
sacr_spec_hid <- nearest_neighbor(weight_func = "rectangular", neighbors = 4) %>%
  set_engine("kknn") %>%
  set_mode("regression")
...
errors_plot <- ggplot(small_sacramento, aes(x = sqft, y = price)) +
  ...
  ylab("Price (USD)") +
  scale_y_continuous(labels = dollar_format()) +
  geom_line(data = sacr_full_preds_hid, aes(x = sqft, y = .pred), color = "blue") + 
  geom_segment(
    data = sacr_new_preds_hid,
    aes(x = sqft, xend = sqft, y = price, yend = .pred), color = "red") + 
  geom_point(data = sacr_new_preds_hid, aes(x = sqft, y = price), color = "black")
...
sacr_wkflw
```
Next we run cross validation for a grid of numbers of neighbours ranging from 1 to 200. 
The following code tunes
the model and returns the RMSPE for each number of neighbours. In the below
results data frame, the `neighbors` variable contains the value of $K$,
the mean (`mean`) contains the value of the RMSPE estimated via cross-validation,
and the standard error (`std_err`) is a measure of how uncertain we are in the mean value. A detailed treatment of this
...
sacr_tunek_plot
```
Figure \@ref(fig:07-choose-k-knn-plot) visualizes how the RMSPE varies with the number of neighbours $K$.
...
```{r 07-howK, echo = FALSE, fig.height = 12, fig.width = 10,fig.cap = "Predicted values for house price (represented as a blue line) from KNN regression models for six different values for $K$"}
...
for (i in 1:6) {
  if (i < 6){
      ...
      plots[[i]] <- ggplot(sacr_preds, aes(x = sqft, y = price)) +
        ...
        ylab("Price (USD)") +
        scale_y_continuous(labels = dollar_format()) +
        geom_hline(data = sacr_preds, mapping = aes(x = sqft), yintercept = mean(sacr_preds$price), color = "blue") +
        ggtitle(paste0("K = ", gridvals[[i]]))
  }
}
...
chapters that this behaviour&mdash;where the model is influenced too much
...
size), depend on many neighbouring observations; in the case where $K$ is equal 
...
the choice of $K$ affects K-nearest neighbours classification, as discussed in the previous
chapter. 
...
In this application, purchase of a home, 
$`r format(round(sacr_summary |> pull(.estimate)), big.mark=",", nsmall=0, scientific=FALSE)` 
is not a terrible error, but it is not that great either. 
The y-axis scale is about $`r format(round(800000), big.mark=",", nsmall=0, scientific=FALSE)`, 
and the error is about 10% of that. 
Depending on someone's budget, a difference of 
$`r format(round(sacr_summary |> pull(.estimate)), big.mark=",", nsmall=0, scientific=FALSE)` 
could make or break whether or not they could afford put an offer on a house. 
At least in the low-mid-range of the data.
...
plot_final <- ggplot() +
  geom_point(data = sacramento_train, mapping = aes(x = sqft, y = price), alpha = 0.4) +
...
## KNN regression with multiple predictors
...
in KNN regression as in KNN classification: more predictors is **not** always
...
We will now demonstrate an analysis of the 
Sacramento real estate \index{Sacramento real estate} data where we perform KNN regression \index{K-nearest neighbors!regression with multiple predictors} with multiple predictors using `tidymodels`. This time we will use
...
If we want to compare this KNN regression model with multiple predictors to the model with only a single
...
The estimated cross validation accuracy for the multiple predictor model is
...
Regardless, let's continue the analysis to see how we can make predictions with a KNN regression model that has multiple predictors
...
```{r 07-knn-mult-viz, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "KNN regression model’s predictions represented as a surface in 3-D space overlaid on top of the data using three predictors (price, house size, and the number of bedrooms)"}
library(plotly)
...
**Strengths:** K-nearest neighbours regression
...
**Weaknesses:** K-nearest neighbours regression

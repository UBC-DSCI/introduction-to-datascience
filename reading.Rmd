...
> silence these messages to help with readability of the book.
...
How did we know to skip 3 lines? We looked at the data! The first three lines
...
> `\t` is an example of an *escape character*, \index{escape character}
...
files. Take a look at what a `.xlsx` file would look like in a text editor: 
...
arguments you need to to load the data into R successfully. If you do not have
...
\index{database} are great when you have large datasets or multiple users
working on a project. There are many relational database management systems,
such as [SQLite](https://www.sqlite.org/index.html),
[MySQL](https://www.mysql.com/), [PostgreSQL](https://www.postgresql.org/),
[Oracle](https://www.oracle.com/ca-en/index.html), and many more. These
...
we can do a lot with big datasets without actually having to bring these vast
...
(*note - this is a synthetic database*).
...
provides a programmatic way to ask for subsets of a dataset. This allows the
...
be presented (e.g. colours, layouts, fonts etc.). 
...
these. Tags are keywords which tell the web browser how to display or format
...
that we want to target (e.g. has a tag name `price`), we can use it to find

...
for (i in 1:3){
    ...
    plt_ctr <- ggplot(marketing_data, aes(y = loyalty, x = csat, colour=label)) +
        ...
        ylim(c(0, 10)) + 
        theme(legend.position = "none") +
        geom_point(data = centers, aes(y = loyalty, x = csat, colour = label), size=10, shape="x") 
...
p1 <- ggplot(assignments, aes(csat, loyalty)) +
  ...
  geom_point(data = clusters %>% mutate(csat = x2, loyalty = x1), aes(colour = cluster), size = 15, shape = "x")
...
> Why is there a "bump" in the total WSSD plot here? Shouldn't total WSSD always
...
> and we found a bad clustering! To fix this, simply try a different seed before clustering.

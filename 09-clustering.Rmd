...
Here we will present an illustrative example using a data set originally from [{palmerpenguins} R data package](https://allisonhorst.github.io/palmerpenguins/). This data set was collected by Dr. Kristen Gorman and the Palmer Station, Antarctica LTER, a member of the Long Term Ecological Research Network [@cancensus2016]. Suppose we have data with two variables measuring customer loyalty and satisfaction, and we want to learn whether there are distinct "types"
of customer. Understanding this might help us develop better products or
...
penguin_data <- data %>% select(flipper_length_mm, bill_length_mm)
...
base <- ggplot(data, aes(x = flipper_length_mm, y = bill_length_mm)) +
  geom_point() +
...
Based on this visualization, we might suspect there are a few subtypes of penguins,
selected from combinations of high/low flipper length and high/low bill length. How
do we find this grouping automatically, and how do we pick the number of subtypes
to look for? The way to rigorously separate the data into groups is to use a clustering algorithm.
...
ggplot(data, aes(y = bill_length_mm, x = flipper_length_mm, colour = cluster)) +
  geom_point() +
  xlab("Flipper Length (mm)") +
  ylab("Bill Length (mm)") 
```
...
Once we have made these determinations, we can use them to inform our species classifications,
...
(``r paste("flipper_length_mm =", round(mean(clus1$flipper_length_mm),2))`` and ``r paste("bill_length_mm =", round(mean(clus1$bill_length_mm),2))``) highlighted below.
...
base <- ggplot(clus1) +
  ...
  xlim(c(min(clus1$flipper_length_mm) - 0.25*sd(clus1$flipper_length_mm), 
         max(clus1$flipper_length_mm) + 0.25*sd(clus1$flipper_length_mm))) +
  ylim(c(min(clus1$bill_length_mm) - 0.25*sd(clus1$bill_length_mm), 
         max(clus1$bill_length_mm) + 0.25*sd(clus1$bill_length_mm))) +
  geom_point(aes(y = mean(bill_length_mm), x = mean(flipper_length_mm), colour="red", size=2.5)) +
...
These distances are denoted by lines for the first cluster of the penguin data example below. 
...
mn <- clus1 %>% summarize(flipper_length_mm = mean(flipper_length_mm), bill_length_mm=mean(bill_length_mm))
for (i in 1:nrow(clus1)) {
  base <- base + geom_segment(x = unlist(mn[1, "flipper_length_mm"]), y = unlist(mn[1, "bill_length_mm"]),
                           xend = unlist(clus1[i, "flipper_length_mm"]), yend = unlist(clus1[i, "bill_length_mm"]))
}
base <- base + geom_point(aes(y = mean(bill_length_mm), x = mean(flipper_length_mm), colour="red", size=2.5)) 
base
```
...
plt_lbl <- ggplot(penguin_data, aes(y = bill_length_mm, x = flipper_length_mm, colour=label)) +
    geom_point(size=2) +
    xlab("Flipper Length (mm)") +
    ylab("Bill Length (mm)") +
   # xlim(c(0, 10)) +
...
for (i in 1:4){
    #compute centers
    centers <- penguin_data %>% group_by(label) %>% summarize_all(funs(mean))
    nclus = nrow(centers)
    #replot with centers
    plt_ctr <- ggplot(penguin_data, aes(y = bill_length_mm, x = flipper_length_mm, colour=label)) +
        geom_point(size=2) +
        xlab("Flipper Length (mm)") +
        ylab("Bill Length (mm)") +
        #xlim(c(0, 10)) +
        #ylim(c(0, 10)) + 
        theme(legend.position = "none") +
        geom_point(data = centers, aes(y = bill_length_mm, x = flipper_length_mm, fill = label), size=5, shape = 21, stroke = 2, colour = "black") 
...
clustering of our customer satisfaction and bill_length_mm data for a number of clusters
ranging from 1 to 9.
...
p1 <- ggplot(assignments, aes(flipper_length_mm, bill_length_mm)) +
  geom_point(aes(color = .cluster, size = 3)) + 
  facet_wrap(~ k) +
  labs(x = 'Flipper Length (mm)', y = 'Bill Length (mm)', colour = 'Cluster') +
  theme(text = element_text(size=20), legend.position = "none") +
  geom_point(data = clusters %>% mutate(flipper_length_mm = x2, bill_length_mm = x1), aes(fill = cluster), colour = "black", size = 5, shape = 21, stroke = 2)
...
unscaled_data <- read_csv('data/penguins.csv') %>%
  select(bill_length_mm, flipper_length_mm)
```
...
clustered_data <- augment(penguin_clust, unscaled_data)
clustered_data
```
...
```
map(penguin_clust_ks, function(k) kmeans(penguin_data, k))
```

The anonymous function in the above call is `function(k) kmeans(penguin_data, k)`.
This function takes a single argument (`k`) and evaluates `kmeans(penguin_data, k)`.
...
```{r 10-choose-k-part2}
penguin_clust_ks <- tibble(k = 1:9) %>%
  mutate(penguin_clusts = map(k, function(ks) kmeans(penguin_data, ks)))
penguin_clust_ks
```
...
```{r 10-choose-k-part3}
penguin_clust_ks <- tibble(k = 1:9) %>%
  mutate(penguin_clusts = map(k, ~kmeans(penguin_data, .x)),
...
penguin_clust_ks <- tibble(k = 1:9) %>%
  mutate(penguin_clusts = map(k, ~kmeans(penguin_data, nstart = 10, .x)),
         glanced = map(penguin_clusts, glance)) 

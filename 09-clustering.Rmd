...
base <- ggplot(clus1) +
  ...
  xlim(c(min(clus1$flipper_length_mm) - 2, max(clus1$flipper_length_mm) + 2)) +
  ylim(c(min(clus1$bill_length_mm) - 1, max(clus1$bill_length_mm) + 1)) +
...
for (i in 1:6) {
  base <- base + geom_segment(x = unlist(mn[1, "flipper_length_mm"]), y = unlist(mn[1, "Bill Length (mm)"]),
                           xend = unlist(clus1[i, "flipper_length_mm"]), yend = unlist(clus1[i, "Bill Length (mm)"]))
}
...
for (i in 1:4){
    ...
    dists <- rbind(centers, penguin_data) %>% 
           select("flipper_length_mm", "Bill Length (mm)") %>%
           dist() %>%
           as.matrix()
...
```{r 10-kmeans}
set.seed(1234)
marketing_clust <- kmeans(penguin_data, centers = 3)
marketing_clust
```
...
clustered_data <- augment(marketing_clust, penguin_data)
clustered_data
```
...
```
map(marketing_clust_ks, function(k) kmeans(penguin_data, k))
```
...
```{r 10-choose-k-part2}
marketing_clust_ks <- tibble(k = 1:9) %>%
  mutate(marketing_clusts = map(k, function(ks) kmeans(penguin_data, ks)))
marketing_clust_ks
```
...
```{r 10-choose-k-part3}
marketing_clust_ks <- tibble(k = 1:9) %>%
  mutate(marketing_clusts = map(k, ~kmeans(penguin_data, .x)),
...
marketing_clust_ks <- tibble(k = 1:9) %>%
  mutate(marketing_clusts = map(k, ~kmeans(penguin_data, nstart = 10, .x)),
         glanced = map(marketing_clusts, glance)) 

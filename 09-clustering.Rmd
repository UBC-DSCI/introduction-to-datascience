...
The K-means algorithm is quite simple. We begin by picking K, and uniformly randomly assigning data to K clusters. 
...
For example, in the customer data example from earlier, our initialization might look like:

Next, we compute the center of each cluster:

Then we reassign each observation to its closest center:

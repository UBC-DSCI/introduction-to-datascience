...
for (i in 1:3){
    ...
    plt_ctr <- ggplot(marketing_data, aes(y = loyalty, x = csat, colour=label)) +
        ...
        ylim(c(0, 10)) + 
        theme(legend.position = "none") +
        geom_point(data = centers, aes(y = loyalty, x = csat, colour = label, size=2.5)) 
...
To solve this problem when clustering data using K-means, we should randomly reinitialize the labels a few times, run K-means for each initialization,
...
kclusts <- tibble(k = 1:9) %>%
  mutate(
    kclust = map(k, ~kmeans(marketing_data[-"labels"], .x)),

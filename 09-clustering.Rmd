...
## Requirements for data pre-processing with K-means

Similar to k-nearest neighbours classification and regression, K-means 
clustering uses a distance metric to decide which points are more similar to 
each other. In K-means similar points typically end up in the same cluster, thus 
things that can affect how close in distance K-means thinks the points are will
have a large impact on which cluster data points end up being assigned to.
Without scaling, any variables that have a large scale will have a much larger 
...
Remember from chapter 6, that standardized data is when all variables in a data 
set have a mean (center) of 0 and a standard deviation (scale) of 1. 

In this chapter, the data we are using are already on the same scale. However, 
if this is not the case for your data (and it is often not), you can use the 
`scale` function in R to do this. We show an example of how to use this function 
below using the data in this chapter:

```{r get-unscaled-data, echo = FALSE, message = FALSE, warning = FALSE}
...
we want to run K-means with (here, 1 to 9). 
...
K. When we do this, we need to use `map` a little bit differently than we have
before. This is because we need to iterate over `k`, which is the second 
argument to the `kmeans` function. In the past we have been iterating over the
first argument to a map function, and that is the default, so we could easily 
write `map(data_frame, function_name)`. This won't work here, as we need to 
provide our data frame as the first argument to the `kmeans` function.

To make this work we will need to use what is called an anonymous function with 
map. An anonymous function is a function that we write ourselves, and that has
no name associated with it. To do this we will write our map statement like 
this:

```
map(k, function(ks) kmeans(marketing_data, ks))
```

What the above `map` call does is for each value in the thing we are iterating 
over (here `k`), it passes it to whatever we name inside of `function()`, here 
`ks`. And then that value is passed into that same name in the code that follows 
`function()` and it is executed. 

Below we execute this `map` call inside of a `mutate` function in our 
...
Next, we use `map` again to then apply `glance` to each of the K-means
...
of [Introduction to Statistical Learning with Applications in R](http://www-bcf.usc.edu/~gareth/ISL/ISLR%20Seventh%20Printing.pdf) [-@james2013introduction] by Gareth James, Daniela Witten, Trevor Hastie and Robert Tibshirani, as well

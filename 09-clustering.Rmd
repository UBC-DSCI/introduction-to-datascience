...
Here we will present an illustrative example using a simulated data set. Suppose we have data with two variables measuring customer loyalty and satisfaction, and we want to learn whether there are distinct "types"
...
```{r 10-kmeans}
set.seed(1234)
penguin_clust <- kmeans(penguin_data, centers = 3)
penguin_clust
```
...
clustered_data <- augment(penguin_clust, penguin_data)
clustered_data
```
...
cluster_plot <- ggplot(clustered_data, 
                       ...
                       size=2) +
  geom_point() +
  labs(x = 'Customer satisfaction', y = 'bill_length_mm', colour = 'Cluster')
cluster_plot
```

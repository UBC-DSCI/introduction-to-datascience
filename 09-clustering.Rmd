...
ggplot(data, aes(y = bill_length_scaled, x = flipper_length_scaled, colour = cluster)) +
  ...
  xlab("Flipper Length (scaled)") +
  ylab("Bill Length (scaled)") + 
  scale_color_manual(values= c("forestgreen", "darkorange", "dodgerblue"))
```
...
- small flipper length and small bill length (<font color="forestgreen">green cluster</font>), 
- small flipper length and large bill length (<font color="darkorange">orange cluster</font>).
- and large flipper length and large bill  length (<font color="dodgerblue">blue cluster</font>).
...
base <- ggplot(clus1) +
  geom_point(aes(y = bill_length_scaled, x = flipper_length_scaled)) +
  labs(x = "Flipper Length (scaled)", y = "Bill Length (scaled)") +
  theme(legend.position = "none") +   scale_color_manual(values= c("forestgreen", "darkorange", "dodgerblue"))
...
plt_lbl <- ggplot(penguin_data, aes(y = bill_length_scaled, x = flipper_length_scaled, colour = label)) +
  ...
  # ylim(c(0, 10)) +
  theme(legend.position = "none") + 
  scale_colour_manual(values= c("forestgreen", "darkorange", "dodgerblue"))
print(plt_lbl)
```
</center>
...
p1 <- ggplot(assignments, aes(flipper_length_scaled, bill_length_scaled)) +
  ...
  geom_point(data = clusters %>% mutate(flipper_length_scaled = x1, bill_length_scaled = x2), aes(fill = cluster), colour = "black", size = 5, shape = 21, stroke = 2)

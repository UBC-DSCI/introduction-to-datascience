...
p1 <- ggplot(assignments, aes(flipper_length_scaled, bill_length_scaled)) +
  ...
  geom_point(data = clusters %>% mutate(flipper_length_scaled = x1, bill_length_scaled = x2), aes(fill = cluster), colour = "black", size = 5, shape = 21, stroke = 2) + scale_fill_manual(values = cbbPalette)

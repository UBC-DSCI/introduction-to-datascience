...
Before we create the plot, we first need to determine what five languages people report speaking as their primary language spoken at home. We will use the `slice_max()` function, where we specify the data frame, the number of rows, and the variable to order by.
```{r}
can_lang_top <- slice_max(can_lang, n = 5, order_by = most_at_home)
can_lang_top
```
We will begin with a bar plot of the `most_at_home` column from our data frame. To create a bar plot of these two variables using the `ggplot` function, we would the following: 

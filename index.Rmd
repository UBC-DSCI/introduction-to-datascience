...
For each of these problems, we map them to the type of data analysis question being asked and discuss what kinds of data are neded to answer such questions. 

<!---FIGURE ON DATA ANALYSIS QUESTIONS TO GO HERE--->

This book/course is structured so that learners spend the first four chapters learning how to use R to load, wrangle/clean and visualize data while answering descriptive and exploratory data analysis questions. The remaining chapters cover solutions to the four problems discussed above, which are useful for answering predictive and inferential data analysis questions. Causal and mechanistic data analysis questions are beyond the scope of this course.
...
There are 52 rows in this dataset (corresponding to the 50 US states, the District of Columbia and the US territory, Puerto Rico). 
...
Creating data visualizations is an essential piece to any data analysis. For the remainder of Chapter 1, we will learn how we can use some of the `tidyverse` functions to make visualizations to explore if there is a relationship between median household income and median propery value across these US regions, as well as whether there is a pattern in which party each state voted for in the 2016 US election. This is an example of an exploratory data analysis question, as we are looking for relationships and patterns within the data set we have, but are not trying to generalize what we find beyond this data set. 
...
From this visualization we see that for the 52 US regions in this data set, as median household income increases so does median property value. When we see two variables do this, we call this a positive relationship. Because of the data point in the lower left-hand corner, drawing a straight line through these points wouldn't fit very well. When a straight-line doesn't fit the data well we say that it's non-linear. Because the increasing pattern is fairly clear (not fuzzy) we can say that the relationship is strong. 

Learning how to describe data visualizations is a very useful skill and we will do this for you in our course (as we did above) until we get to Chapter 4 which focuses on data visualization. There we will explicitly teach you how to do this yourself, and how to not over-state or over-interpret the results from a data visualization.

### Coloring points by group
Remembering the second part of our exploratory data analysis question: when considering the relationship between median household income and median property value, **is there a pattern in which party each state voted for in the 2016 US election?** One common way to explore this is to colour the data points on a scatter plot by a group/category in the data set. For example,
...
```{r scatter colour by party, fig.width=5.75, fig.height=4, warning=FALSE}
...
```{r scatter colour by party no PR, fig.width=5.75, fig.height=4, warning=FALSE}
...
Does this mean that rich states usually vote for Democrats and poorer states generally vote for Republicans? Or could we use this data visualization on its own to predict who each state will elect in the next presidential election? The answer to both these questions is no. What we can do with this exploratory data analysis is use it to create new hypotheses and ideas to pose new questions (like the ones at the beginning of this paragraph). Answering those questions would likely involve gathering additional data and doing more complex analyses, which we will see some examples of later in this course.
...
```{r nachos to cheesecake, fig.width=5.75, fig.height=4, warning=FALSE, message=FALSE}
library(tidyverse)

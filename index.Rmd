...
"Aboriginal languages" category. We can use the `filter` function to obtain
...
### Using `arrange` and `slice` to order and select rows
...
Next we will use the `slice` function, which selects rows according to their row number. Since we want to choose the ten Aboriginal languages, we will indicate we want the rows 1 to 10 (`1:10`).
```{r}
ten_lang <- slice(arranged_lang, 1:10)
ten_lang
```
Now we have answered our initial question with the frequency table we generated in R. Are we done? Well technically this table does answer our question. However, we can go one step further and create a visualization to answer our question as well. Visualizations are a great tool for summarizing information into a clear format to help you effectively communicate with your audience. 

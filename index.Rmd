...
Before the arrival of European colonists, Indigenous peoples lived in Canada with their own cultures and many different languages. Many Aboriginal languages are unique to Canada and not spoken anywhere else in the world. (https://www150.statcan.gc.ca/n1/pub/11-630-x/11-630-x2018001-eng.htm) Sadly, colonization has led to the loss of many of these languages. For instance, generations of children were not allowed to speak their mother tongue (the first language an individual learns in childhood) in Canadian residential schools. Colonizers renamed places they had "discovered". Acts such as these have significantly harmed the continuity of Aboriginal languages in Canada. https://opentextbc.ca/indigenizationfoundations/chapter/43/#:~:text=In%20Canada%2C%20colonization%20occurred%20when,not%20favour%20the%20Indigenous%20Peoples.

> To learn more, see this resource: 

In the 2016 census, more than 70 Aboriginal languages were reported as being spoken. Suppose we want to ask the question:
...
This question is an example of a descriptive question since we are summarizing characteristics of a data set without interpretation. Now that we have asked our question, the next step is to collect or identify the data set you will be working with. 
...
Now we just need to use R to load the data, do some wrangling and visualize the data to answer this question!
...
This file--- originally from [{canlang} R data package](https://ttimbers.github.io/canlang/) [@timbers2020canlang] ---has language data collected in the 2016 Canadian census [@cancensus2016]. 
...
Object names should consist of letters, numbers and underscores `_` to separate words within a name.
You should also try to give your objects meaningful names.
For instance, you *can* name a plot `x`. 
However, using more meaningful terms, such as `canlang_plot`, will help you remember what each name in your code represents. 

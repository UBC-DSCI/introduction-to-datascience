...
In this book, we define *data science* as the study and development of reproducible, 
auditable processes to obtain value (i.e., insight) from data. 
...
## Ask a question 
Where to begin? As outlined in Figure \@ref(fig:img-chapter-overview), we will begin
with a question we want to answer. Generally, we need some context, which will frame our question. 
...
schools. Colonizers renamed places they had "discovered" [@wilson2018]. Acts such as these have significantly harmed the continuity of Aboriginal languages in Canada.
...
Now that we have asked our question and identified the data we are using, we just need to use R to load the data, do some wrangling and visualize the data to answer this question!
...
Object names should consist of letters, numbers and underscores `_` to separate
the words in a name.
You should also try to give your objects meaningful names.
For instance, you *can* name a plot `x`. 
However, using more meaningful terms, such as `language_plot`, will help you remember what each name in your code represents. 
...
Looking at our `can_lang` data, we see that the column `category` contains 
...
want to filter our data set so we restrict our attention to only the 
"Aboriginal languages" category. 

We can use the `filter` function to obtain
the subset of rows with desired values from a data frame. Again, our first 
argument is the name of the data frame object, `can_lang`. The second argument
is a logical statement to use when filtering the rows. Suppose we were interested 
...
Now let's use `select` to extract the `language` and `mother_tongue` columns from this data frame. To do this, we need to provide the `select` function with three arguments. The first argument is the 
...
The  `arrange` function allows us to order the rows of a data frame by the values of a selected column. We need to pass the data frame, and the variable to order by in the first two arguments of this function.  
Since we want to choose the ten Aboriginal languages most often reported as a mother tongue language, we will use the `arrange` function to order the rows in our `selected_lang` data frame by the `mother_tongue` column. We want to order the rows in descending order (from largest to smallest) so we include `desc` before the column name `mother_tongue` to sort in descending order. 

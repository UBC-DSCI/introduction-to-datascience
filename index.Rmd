...
Four common data science problems that are useful for answering predictive and inferential data analysis questions covered in this book are:
...
Taking another look at our data set below, we can immediately see that the three columns we are interested in visualizing---mother tongue (`mother_tongue`), language spoken most at home (`most_at_home`), and higher level language category (`category`)---are all in separate columns. In addition, there is a single row (or observation) for each language. 
The data are therefore in what we call a *tidy data* format. 
Tidy data is particularly important concept and will be a major focus in the remainder of this course: many of the functions from `tidyverse` require tidy data, including the `ggplot` function that we will use shortly for our visualization. We will formally introduce this concept in chapter 3.

...
(i.e. restructure the data, so it's in a useable format) and visualize data while
...
We map each of these methods to the broader data analysis question, listed in Table \@ref(tab:questions-table), they help answer and discuss what kinds of data are needed to answer such questions [@leek2015question; @peng2015art]. Causal or mechanistic questions in Table \@ref(tab:questions-table) are beyond the scope of this book.
...
Now that we've learned about different types of questions. We need some context with which to frame our question. 
...
the characteristics of a data set without further interpretation. To answer this question, we may want to create a table or a visualization that shows us these ten languages and their associated counts. Now that we have asked our question, the next step is collecting or identifying the data set with which we will be working. 
...
Almost always, the first step in data analysis is to load a data set into R. A data set is a collection of numbers or values and can come in many different forms! Tables in Microsoft Excel are an example of rectangular-shaped, spreadsheet-like data as shown in Figure \@ref(fig:img-spreadsheet-vs-dataframe).
When we load this type of data into R, it is represented as a *data frame* object. Figure \@ref(fig:img-spreadsheet-vs-dataframe) shows an R data frame is very similar to a spreadsheet where the rows are the collected observations (e.g. students), and the columns are the characteristics of those observations (e.g. grades). 
...
functions. The `read_csv` function, in particular, can be made accessible through the

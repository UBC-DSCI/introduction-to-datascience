...
This textbook aims to introduce readers to data science. In this book, we define *data science* as the study and development of reproducible, auditable processes to obtain value (i.e., insight) from data. At a high level, in this book, readers will learn how to (1) use reproducible tools to do data analysis and (2) identify and solve common problems in data science. The book is structured, so learners spend the first four chapters learning how to use the R programming language to load, wrangle, clean, and visualize data while answering descriptive and exploratory data analysis questions. The following six chapters illustrate how to solve common problems in data science, which help answer predictive and inferential data analysis questions. In the final chapter, we discuss how to set up your own computer with tools like R and Jupyter. For readers who need help setting up their system on their computer, we recommend starting with the [Setting up your own computer] chapter before reading chapter 2. Figure \@ref(fig:img-chapter-overview) summarizes a data science pipeline and what you will learn in each chapter of this book.
...
The four common data science problems that are useful for answering predictive and inferential data analysis questions covered in this book are:
...
To load this data into R and then do anything else with it afterwards, we will need to use a *function.* 
...
to be able to use it, we have to load it from somewhere else: a collection of useful functions known as a *package*. The `read_csv` function, in particular, is in the `tidyverse` package (more on this later), which we load using the `library` function.
...
functions from the `tidyverse` to make visualizations that let us explore relationships in data. Before we start creating visualizations, we should take a closer look at the language data collected in the 2016 Canadian census to familiarize ourselves with it.
...
In this data frame, there are 214 rows (corresponding to the 214 languages recorded on the 2016 Canadian census) and six columns: 
...
6. `lang_known`: Total count of Canadians from the Census who reported knowledge of the language for the population in private households.

We will now develop a visualization of the language data collected in the 2016 Canadian census we've been working with that will help us understand two potential relationships in the data. First, we will explore the relationship between the number of people who speak a language as their mother tongue and the number who speak that language as their primary language spoken at home. Then we will see whether there is a pattern in the strength of this relationship in the higher-level language categories (Official languages, Aboriginal languages, or non-official and non-Aboriginal languages). These are examples of exploratory data analysis questions because we are looking for relationships and patterns within the data set we have but are not trying to generalize what we find beyond this data set.  

### Using `ggplot` to create a scatter plot


Looking again at our data set below, we can immediately see that the three columns we are interested in visualizing---mother tongue (`mother_tongue`), language spoken most at home (`most_at_home`), and higher-level language category (`category`)---are all in separate columns. In addition, there is a single row (or observation) for each language. The data are, therefore, in what we call a *tidy data* format. 
Tidy data is a fundamental concept and will be a significant focus in the remainder of this course: many of the functions from `tidyverse` require tidy data, including the `ggplot` function that we will use shortly for our visualization. We will formally introduce this concept in chapter 3.
...
To create a scatter plot of these two variables using the `ggplot` function, we would the following: 

```{r img-ggplot, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Creating a scatter plot with the ggplot function", out.width="1100", fig.retina = 2}
knitr::include_graphics("img/function_ggplot.jpeg")
```

Below we create a scatter plot using the `ggplot` function as shown in figure \@ref(fig:img-ggplot):
...
In figure \@ref(fig:mother-tongue-vs-most-at-home-labs), most of the 214 observations in this data set are bunched up in the lower left-hand side of this visualization because many people in Canada speak the two official languages (English and French). Thus to answer our question, we will need to adjust the scale of the x and y axes so that they are on a log scale.
...
From figure \@ref(fig:mother-tongue-vs-most-at-home-scale), we see that for the 214 languages in this data set, as the number of people who have a language as their mother tongue increases, so does the number of people who speak that language at home. When we see 
...
knitr::include_graphics("img/function_mutate.jpeg")
```
...
In the code chunk below, we put everything from this chapter together. We have added a few more layers to make the data visualization even more effective. Specifically, we have improved the visualization's accessibility by choosing colours that are easier to distinguish. We also mapped category to shape, handled the overlapping data points by making them slightly transparent, and changed the background from grey to white to improve the contrast. 

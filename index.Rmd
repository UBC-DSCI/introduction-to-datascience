...
Before we create the plot, we first need to determine what are the top five languages people report speaking as their primary language spoken at home. We will use the `slice_max()` function, where we specify the data frame, the number of rows, and the variable to order by.
```{r}
can_lang_top <- slice_max(can_lang, n = 5, order_by = most_at_home)
can_lang_top
```
We will begin with a bar plot using the `most_at_home` and `language` columns from our data frame. To create a bar plot of these two variables using the `ggplot` function, we would the following: 
...
Below we create a bar plot using the `ggplot` function as shown in figure \@ref(fig:img-ggplot):

```{r bar-most-at-home, fig.width=5.75, fig.height=5, warning=FALSE, fig.cap = "Bar plot of number of Canadians reporting a language as their primary language at home"}
ggplot(can_lang_top, aes(x = language, y = most_at_home)) +

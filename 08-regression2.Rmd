...
and characterizes its strengths and weaknesses in comparison to K-NN regression. The focus is, as usual,
on the case where there is a single predictor and single response variable of interest; but the chapter
concludes with an example of how to use *multivariate linear regression* when there is more than one
predictor.
...
> the word *regression* there...). After reading this chapter, if you want something like
...
Let's return to the Sacramento housing data from the previous chapter to see how
to apply linear regression and how it compares to K-NN regression. For now, we will consider 

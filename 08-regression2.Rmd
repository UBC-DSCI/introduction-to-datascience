...
Note once again that have the same concerns regarding multiple predictors
...
on the quality of predictions. Because the same predictor selection 
algorithm from the classification chapter extends to the setting of linear regression,
we will not cover it again in this chapter.
...
- show coefficients get unstable (motivate with negative bedroom coeff, use samples to illustrate)
- show outliers can mess with predictions (use a single sample, keep/remove outliers)
- show that knn is less bothered by outliers (since predictions are local)

...
through all of our training observations. 
...
The problem is that if we used a different training data set of house prices and sizes
...
is `r format(round(sacr_summary %>% filter(.metric == 'rmse') %>% pull(.estimate)), big.mark=",", nsmall=0, scientific=FALSE)`. 
But what does this RMSPE score mean? When we calculated test set prediction accuracy in KNN
...
of `r format(round(knn_mult_mets %>% filter(.metric == 'rmse') %>% pull(.estimate)), big.mark=",", nsmall=0, scientific=FALSE)`.

...
As with KNN classification (or any prediction algorithm for that manner), KNN regression has both strengths and weaknesses. Some are listed here:
...
In order to evaluate the quality of the multivariate KNN model on the test data,
we need to re-train the model on the entire
training data set, and then use that model to make predictions
on the test data.
```{r 07-re-train}

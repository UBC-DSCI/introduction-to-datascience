...
regression we need to use the `preProcess` function in R on our predictors to ensure
this doesn't happen.
...
We begin by splitting our data into train and test:
...
Further, looking at the data above, we can see that `sqft` and `beds` (number of bedrooms) are on vastly different scales. Thus we need to
compute a scaling using the `preProcess` function on the training inputs, and
apply the scaling to both the train and test inputs:

```{r 09-mult-test-scaling}
scale_transformer <- preProcess(X_train, method = c("scale"))

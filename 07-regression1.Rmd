...
```{r 07-small-eda-regr, fig.height = 4, fig.width = 5, fig.cap = "Scatter plot of price (USD) versus house size (square footage) with vertical line indicating 2,000 square feet on x-axis"}
...
```{r 07-predictedViz-knn, echo = FALSE, fig.height = 4, fig.width = 5, fig.cap = "Scatter plot of price (USD) versus house size (square footage) with predicted price for a 2,000 square-foot house based on 5 nearest neighbours represented as a red dot"}
nn_plot +
...
our K-NN regression model for 6 different values for $K$: 1, 3, `r kmin`, 41, 250, and 932 (i.e., the entire dataset).
...
number of bedrooms increase, so does sale price. Does adding the number of bedrooms
...
If we want to compare this multivariate K-NN regression model to the model with only a single
predictor *as part of the model tuning process* (i.e., to decide which one we should use to make future predictions),
...
In order to evaluate the quality of the multivariate K-NN model on the test data,

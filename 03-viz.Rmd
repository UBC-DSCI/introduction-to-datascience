...
- the *geometric object*: specifies how the mapped data should be displayed
 ...
 We can see in Figure \@ref(fig:03-ggplot-function-scatter) how each of these aspects maps to our code for creating a basic scatter plot for our `co2_df` data. 
...
We will return to the `canlang` data set, which contains counts of languages from the 2016 Canadian census.
...
ggplot(can_lang, aes(x = most_at_home, y = mother_tongue)) +
  geom_point()
``` 
To improve the interpretability of the data visualization that we created we should replace the default axis names with a more
informative labels. 
...
Most of the data points from the 214 observations in this data set are bunched up in the lower left-handside of the visualization in Figure \@ref(fig:mother-tongue-vs-most-at-home-labs). The data clumped is because many more people in Canada speak the the two languages represented by the points in the top right corner. We can confirm the two points in the upper right corner of the visualization are the two official languages (English and French) by filtering the data:
...
people reported that their mother tongue was English in the 2016 Canadian census? To really understand this number, we need context. In particular, how many people were in Canada when this data was collected? From the 2016 Canadian census profile, we can see that the population was reported to be `r format(census_popn, scientific = FALSE, big.mark = ",")` people. The count of 
...
```{r}
can_lang <- can_lang %>% mutate(mother_tongue = mother_tongue / 35151728, most_at_home = most_at_home / 35151728)
can_lang
```
...
Changing the units to include this context increases our understanding and allows us to interpret the numbers in our data set better.
...
scatter plot we have already created by group/category. For example, given that we have the higher level language category for each language recorded in the 2016 Canadian census, we can colour the points in our previous 
scatter plot to represent each language's higher-level language category.

We will add an argument to the `aes` function, specifying that the points should be coloured by the `category` column:
...
ggplot(can_lang, aes(x = most_at_home, y = mother_tongue, color = category)) +
  ...
  scale_y_log10(labels = scales::comma)
```
From the visualization in Figure \@ref(fig:scatter-colour-by-category), we can now clearly see that not just a lot, but that the majority of Canadians reported one of the official languages (English and French) as their mother tongue and as the language they speak most often at home. 
...
ggplot(can_lang, aes(
  ...
  scale_color_manual(values = c("deepskyblue2", "firebrick1", "forestgreen", "black")) + # choose point colours
  xlab("Language spoken most at home (proportion of Canadian residents)") +
  ylab("Mother tongue (proportion of Canadian residents)") +
  scale_x_log10(labels = scales::comma) +
  scale_y_log10(labels = scales::comma) 

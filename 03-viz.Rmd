...
There are external references that contain a wealth of additional information on the topic of data visualization:
...
- see Grolemund & Wickham's [R for Data Science](https://r4ds.had.co.nz/) chapter on [creating visualizations using `ggplot2`](https://r4ds.had.co.nz/data-visualisation.html) for a deeper introduction into the syntax and grammar of plotting with `ggplot2` specifically
- see the [ggplot2 reference](https://ggplot2.tidyverse.org/reference/) for a list of useful `ggplot2` functions
...
- `ggsave` and different output formats
...
- **scatter plots** visualize the distribution of data on two axes
...
Some rules of thumb for doing this are listed below. They generally fall into two classes: you want to make refinements that make your
visualization *convey your message / answer the question more clearly*, as well as those that *simplify the visualization / reduce visual noise*.
...
- Make sure to use colourschemes that are understandable by those with colourblindness (a surprisingly large fraction of the 
  ...
  has a tool to pick such colourschemes, and you can check your visualizations after
...
This section will cover examples of how to choose a visualization given a dataset and a question that you want to answer, 
and then how to create the visualization in R using `ggplot2`.

### The Mauna Loa CO2 dataset 
 This dataset contains the atmospheric concentration of carbon dioxide (CO2, in parts per million) at the Mauna Loa research station in Hawaii 
...
However, since time is an ordered quantity, we can try using a line plot instead using the `geom_line` geometric object. Line plots require
...
```{r 03-data-co2-line-2-print, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}
print(co2_line)
```

### The island landmass dataset
This dataset contains a list of Earth's land masses as well as their area (in thousands of square miles). **Question:** Are the continents (North / South America, Africa, Europe, Asia, Australia, Antarctica) Earth's 7 largest landmasses? If so, what are the next few largest landmasses after those?
```{r 03-data-islands}
# islands data 
# (with some light wrangling to make it a dataframe with informative column names)
...
question is a bar plot, specified by the `geom_bar` geometric object in `ggplot2`. However, by default, `geom_bar` sets the heights
...
```{r 03-data-islands-bar}
islands_bar <- ggplot(islands_df, aes(x = landmass, y=size)) + geom_bar(stat="identity")
```

```{r 03-data-islands-bar-print, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}
print(islands_bar)
```
...
```{r 03-data-islands-bar-2}
islands_top12 <- top_n(islands_df, 12, size)
islands_bar <- ggplot(islands_top12, aes(x = landmass, y=size)) + geom_bar(stat="identity") + coord_flip()

```

```{r 03-data-islands-bar-2-print, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}
print(islands_bar)
```
...
```{r 03-data-islands-bar-4-print, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}
print(islands_bar)
```
...
### The Old Faithful eruption / waiting time dataset
This dataset contains measurements of the waiting time between eruptions and the subsequent eruption duration (in minutes). **Question:** Is there a 
...
```{r 03-data-faithful-scatter-2}
faithful_scatter <- ggplot(faithful, aes(x = waiting, y = eruptions)) + 
                         geom_point() +
                         labs(x = 'Waiting Time (mins)', y = 'Eruption Duration (mins)') +
                         theme(text = element_text(size = 18))
```
```{r 03-data-faithful-scatter-2-print, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}
print(faithful_scatter)
```


### The Michelson speed of light dataset
This dataset contains measurements of the speed of light (in kilometres per second with 299,000 subtracted) from the year 1879 
...
In this experimental data, Michelson was trying to measure just a single quantitative number (the speed of light). The dataset
contains many measurements of this single quantity. To tell how accurate the experiments were, we need to visualize the 
distribution of the measurements. We can do this using a *histogram*. A histogram helps us visualize how a 
particular variable is distributed in a dataset by separating the data into bins, and then using vertical bars 
...
```{r 03-data-morley-hist}
morley_hist <- ggplot(morley, aes(x = Speed)) + geom_histogram() 
```

```{r 03-data-morley-hist-print, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}
print(morley_hist)
```
...
In order to visualize the true speed of light, we will add a vertical line with the `geom_vline` geometric object, setting the `xintercept` argument
to the true value:

```{r 03-data-morley-hist-2}
morley_hist <- ggplot(morley, aes(x = Speed)) + 
                 geom_histogram() +
                 geom_vline(xintercept = 792.458, linetype="dashed", size=1.0)
```

```{r 03-data-morley-hist-2-print, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}
print(morley_hist)
```
...
```{r 03-data-morley-hist-3}
morley_hist <- ggplot(morley, aes(x = Speed, fill=factor(Expt))) + 
                 geom_histogram(position="identity", alpha=0.5) +
                 geom_vline(xintercept = 792.458, linetype="dashed", size=1.0)
```

```{r 03-data-morley-hist-3-print, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}
print(morley_hist)
```
...
```{r 03-data-morley-hist-4-print, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}
print(morley_hist)
```
...
```{r 03-data-morley-hist-5-print, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}
print(morley_hist)
```

Wow, impressive! These measurements of the speed of light from 1876 had errors around *0.05%* of the true speed. This shows
...
3) Answer the question using your visualization. You can focus on different aspects of it:
  - **trends (lines):** linear vs nonlinear
  - **distributions (scatters, histograms):**
  - **amounts (bars):**
4) Summarize your findings, and use them to motivate whatever you will discuss next.

[Chapter 29 of the Fundamentals of Data Visualization](https://serialmentor.com/dataviz/telling-a-story.html) is an excellent resource for this.

**Mauna Loa Atmospheric CO2 Measurements:** Lorem ipsum dolor sit amet.

**Island Landmasses:** Lorem ipsum dolor sit amet.

**Michelson Light Speed Experiments:** Lorem ipsum dolor sit amet.

**Old Faithful Eruption / Waiting Times:** Lorem ipsum dolor sit amet.

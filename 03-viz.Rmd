...
This dataset contains a list of Earth's land masses as well as their area (in thousands of square miles). **Question:** Are the continents (North / South America, Africa, Europe, Asia, Australia, Antarctica) Earth's 7 largest landmasses? What are the next few largest landmasses after those?
```{r 03-data-islands}
# islands data 
# (with some light wrangling to make it a dataframe with informative column names)
...
Alright, not bad! This is definitely the right kind of visualization, as we can clearly see and compare the sizes of roughly the top 10
landmasses. The major issues are that the sizes of the smaller landmasses are hard to distinguish, and that the names of the landmasses 
...
islands_bar <- ggplot(islands_top12, aes(x = landmass, y=size)) + geom_bar(stat="identity")
```
...
to take the top 12 largest landmasses rearrange the `x` axis by `-size` (the negative sign specifies decreasing order) `arrange` and `desc` functions to arrange the
rows in decreasing order by size, and then `head` to keep only the 12 first rows:

```{r 03-data-islands-bar-3}
islands_top12 <- top_n(islands_df, 12, size)
islands_bar <- ggplot(islands_top12, aes(x = reorder(landmass, size), y = size)) + geom_bar(stat="identity") +coord_flip()
```

```{r 03-data-islands-bar-3-print, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}
print(islands_bar)
```


Since we want to color too, we use `labs`
...
islands_bar <- ggplot(islands_ct, aes(x = reorder(landmass, size), y = size, fill = is_continent)) + 
                   ...
                   labs(x = 'Landmass', y = 'Size (1000 square km)', fill = 'Type') +
                   ylab('Size (1000 km)') + 
                   coord_flip() +
                   theme(text = element_text(size = 18)) 
```
...
One last thing: labels aren't in the right order, and the plot was squished by the legend

```{r 03-data-islands-bar-5}
islands_top12 <- top_n(islands_df, 12, size)
continents <- c('Africa', 'Antarctica', 'Asia', 'Australia', 'Europe', 'North America', 'South America')
islands_ct <- mutate(islands_top12, is_continent = ifelse(landmass %in% continents, 'Continent', 'Other'))
islands_bar <- ggplot(islands_ct, aes(x = reorder(landmass, size), y = size, fill = is_continent)) + 
...
```{r 03-data-islands-bar-5-print, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}
print(islands_bar)
```
...
We can see that the data tend to fall into two groups: one with a short waiting and eruption times, and one with long waiting and eruption times. Note that in this case, there is no overplotting: the points are generally nicely visually separated, and the pattern they form is clear.



### The Michelson speed of light dataset
This dataset contains measurements of the speed of light (in kilometers per second with 299,000 subtracted) from the year 1879 
for 5 experiments, each with 20 consecutive runs. **Question:** Given what we know now about the speed of 
light (299,792.458 kilometers per second), how accurate were the experiments?
```{r 03-data-morley}
...
morley_hist2 <- ggplot(morley, aes(x=Speed, color=factor(Expt), fill=factor(Expt))) + geom_histogram(alpha=0.5, position='identity')

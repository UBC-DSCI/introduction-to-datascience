...
Here we want to distinguish the values according to their `category` group. 
We can add an argument to the `aes` function, specifying that the `category` column should colour the points. This will colour the points according to their group and add a legend at the side of the plot. 
...
The points are coloured according to the default `ggplot2` color palette. What if you want different colors? We can do this! In R, one of the packages that provides alternative color palettes is the `RColorBrewer` library [@RColorBrewer]. You can also print out a list of color blind friendly palettes 
...
You can also use this color blindness simulator to check if your visualizations are color blind friendly

...
Since we are investigating a relationship between two variables (CO2 concentration and date), it may be a good idea to use a scatter plot. 
Scatter plots show the data as individual points with `x` (horizonal axis) and `y` (vertical axis) coordinates. Here, we will use the date
as the `x` coordinate and CO2 concentration as the `y` coordinate.

When using the `ggplot2` library, we create the plot object with the `ggplot` function. The first argument is the dataframe we are working with (here `co2_df`), 
...
that the data are listed in order, and connect the sequence of `x` and `y` coordinates with line segments. Let's again try this with just 
...
relationship between the waiting time before an eruption to the duration of the eruption? Also, would it be possible to use the current time since
...
faithful_scatter <- ggplot(faithful, aes(x = waiting, y = eruptions)) + geom_point()  + geom_density_2d()
```
...
head(morley)
```
**Question:** How accurate are all the experiments?

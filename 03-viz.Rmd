...
- List rules of thumb for creating effective visualizations 
- Use the `ggplot2` library in R to create the above visualizations
- Define the three key aspects of ggplot objects:
     - aesthetic mappings
     - geometric objects
     - scales
- Describe the difference in raster and vector output formats, and use `ggsave` to save visualizations
...
visualization convey your message / answer the question more clearly*, and you want to *simplify the visualization / reduce visual noise*
as much as possible.
...
This section will cover examples of how to choose and refine a visualization given a dataset and a question that you want to answer, 
...
```{r 03-data-co2-line-3-print, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}
print(co2_line)
```
...
3) Answer the question using your visualization. Make sure you describe *all* aspects of the visualization (including describing the axes). But you 
   can emphasize different aspects based on what is important to answering your question:
    ...
    - **distributions (scatters, histograms):** How spread out is the distribution? Where is it centered, roughly? Are there any obvious "clusters" or "subgroups", which would be visible as multiple bumps in the histogram?  
...
more heat from the sun, leading to global warming. (2) In order to assess how quickly the atmospheric concentration of CO2 is increasing over time, we (3) used a dataset from
...
worth further investigation into the differences between these experiments to see why they produced different result distributions.

...
There are many user interfaces for using Git, 
here we will focus on how to use the Jupyter Git interface. 
We will focus on the most common version control operations used day-to-day 
in a standard data science project. 
...
and why data analysis projects can benefit from it
- Create a remote version control repository on GitHub
- use Jupyter's Git version control tools for project versioning and 
...
Version control is system which records changes to a document, 
...
mind-boggling filenames that make it unclear 
which is the current working version of the file 
(e.g., `document_final_draft_final.txt`, `golden_master_final.txt`, 
`to_hand_in_final.txt`, etc), and more. 
Additionally, the iterative nature of data analysis projects 
makes it important to be able to examine earlier versions of code and writing. 
...
This provides the means both to view earlier versions of the project 
and to revert changes. Version control also facilitates collaboration 
...
In the 
[additional resources section at the end of the chapter](\@ref(vc-add-res)) 
...
(*e.g.,* Git) that we would like to record a version of the project. 
...
(who made the changes, when they were made, *etc.*). 
...
it is instead a conceptual placeholder for these files until they are commit.
...
```{r vc-ba2-add, fig.cap = 'The modified files for which you want a snapshot need to be **added** to the staging area.', fig.retina = 2}
...
collaborators (which included future you) about the changes that were made 
...
If your collaborators send their own commits to the GitHub repository, 
they will only initially exist there 
and not on your own machine (Figure \@ref(fig:vc2-changes)).
...
you sync your local repository to what is present on GitHub.  
...
```{r new-repository-02, fig.cap = 'Respository configuration for a project that is public and initialized with a README.md template file.', fig.retina = 2}
...
*Do not* upload your whole project in an archive file (e.g. `.zip`), because
...
- [Good enough practices in scientific computing](https://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1005510#sec014) [-@wilson2014best]  by Greg Wilson, Jennifer Bryan, Karen Cranston, Justin Kitzes, Lex Nederbragt & Tracy K. Teal advises and elaborates further on useful workflows and good enough practices to version control data analysis projects.

### List of common version control systems and repository hosting services
Below we list and link out to the most commonly used version control systems 
and repository hosting services:
...
Below we link to additional tutorials 
and guides for three common repository hosting services:

...
paths: relative paths and absolute paths. A relative path is where the file is with respect to where you currently are on the computer (e.g., where the file that you're working in is). On the other hand, an absolute path is where the file is in respect to the base (or root) folder of 
...
In the additional resources section, we include a link to a short video on the difference between absolute and relative paths. We also include a link to the `here` package, which is a robust method for finding file paths.  
...
Your file could be stored locally, as we discussed, or it could also be somewhere on the internet (remotely). A *Uniform Resource Locator (URL)* (web address) indicates the location of a resource on the internet and helps us retrieve that resource. Next, we will discuss how to get both locally or remotely stored data into R. 

## Reading tabular data from a plain text file into R

### `read_csv` to read in comma-separated files

Now that we have learned about *where* data could be, we will learn about *how* to import data into R using various functions. Specifically, we will learn how to *read* tabular data from a plain text file (a document containing only text) into R and *write* tabular data to a file from R. The function we use to do this depends on the file's format. For example, the last chapter, we learned about using the `tidyverse` `read_csv` function when reading .csv (*c*omma-*s*eparated *v*alues) files. In that case, the separator or *delimiter* that divided our columns was a comma (`,`). We only learned the case where the data matched the expected defaults of the `read_csv` function 
(column names are present, and commas are used as the delimiter between columns). In this section, we will learn how to read 
...
To successfully read data like this into R, the `skip` argument can be useful to tell R how many lines to skip before it should start reading in the data. In the example above, we would set this value to 3. How did we know to skip 3 lines? We looked at the data! Open your data in a text editor and look at the file to decide how many lines to skip. 
...
We can also use `read_csv()` or `read_delim()` (and related functions) to read in tabular data directly from a **U**niform **R**esource **L**ocator (URL) that contains tabular data. In this case, we provide the URL to `read_csv()` as the path to the file instead of a path to a local file on our computer. Similar to when we specify a path on our local computer, here we need to surround the URL by quotes. All other arguments that we use are the same as when using these functions with a local file on our computer.
...
delimiters are, and if there are lines you need to skip. You should do this yourself when trying to read in data files.
...
In Table \@ref(tab:read-table) we summarize the `read_*` functions we covered in this chapter. We also include some other handy functions that read in different file formats, such as `read_tsv` as another way to read in tab-separated data and `read_csv2` for data separated by semicolons `;`. 
...
Another very common form of data storage is the relational database. Databases are great when you have large datasets or multiple users working on a project. There are many relational database management systems, such as
...
> Warning: Usually, databases are very big! Reading the object into your local machine may give an error or take a lot of time to run so be careful if you plan to do this! This is why we may want to filter rows or select columns first before reading it in!
...
**P**rogramming **I**nterface (API), which provides a programmatic way to ask
...
directly from what a website displays is called *web scraping* (or sometimes *screen scraping*). Now, 
...
that data programmatically---in the form of **h**yper**t**ext **m**arkup **l**anguage (HTML) 
and **c**ascading **s**tyle **s**heet (CSS) code---and
process it to extract useful information.
...
website and search for one-bedroom apartments, this is what we are shown:
...
its data between opening and closing tags like these. Above you can see that
...
The above was a particularly simple example of "finding the pattern to look for"; many 
...
After installing and enabling the tool, you can simply click the 
...
> this interesting blog post titled ["Web Scraping and Crawling Are Perfectly
...
the SelectorGadget tool. Make sure to surround the selectors with quotations; `html_nodes` expects that
argument to be a string. The `html_nodes` function then selects *nodes* from the HTML document that 
match the CSS selectors you specified.
A *node* is an HTML tag pair combined with the content stored between the tags. For our
...
to build the list of selectors while maintaining code readability; this avoids
...
```{r 01-tidyverse-twitter, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "The Tidyverse Twitter feed. [Source](https://twitter.com/tidyverse?lang=en)", fig.retina = 2, out.width="800"}
...
After checking the website, it seems like asking for 400 tweets one time is acceptable.
...
```{r 01-tidyverse-authorize, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "The rtweet authorization prompt.", fig.retina = 2, out.width="800"}
...
tidyverse_tweets
```
The data has quite a few variables! Let's reduce this down to a few variables of interest: `created_at`,  `retweet_screen_name`, `is_retweet`, and `text`,.

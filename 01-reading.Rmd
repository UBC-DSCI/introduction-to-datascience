...
- (*optional*) obtain data using APIs and web scraping
...
> This section is not required reading for the remainder of the textbook.
...
data (especially large amounts of data) just plainly available for download
...
into your browser window, your browser asks the web server to give it some data, 
...
that data programmatically---in the form of **h**yper**t**ext **m**arkup **l**anguage (HTML) code---and
...
and using the [`rtweet` R package](https://github.com/ropensci/rtweet)
to access the [Twitter](www.twitter.com) API.
...
Here is a short video on how to install and use the SelectorGadget tool to get
a CSS selector for use in web scraping:
...
After installing and using SelectorGadget as shown in the video, we
obtain the two CSS selectors `.housing` and `.result-price`; we can use
these to 
scrape information about the square footage and the rental price, respectively.
...
and it does not mention that web scraping is disallowed. In the video below, 
we demonstrate how to use the SelectorGadget tool to get CSS Selectors 
from [Wikipedia's Canada](https://en.wikipedia.org/wiki/Canada) for a table that
contains city names and populations from the 2016 Canadian Census: 
...
We find that we should use the following CSS selector list:
```
...
(`\n`). In the next chapter, we will learn more about how to *wrangle* this data
into a more useful format for data analysis using R.
...
library(rtweet)
```
This package provides an extensive set of functions to search Twitter for tweets, users, their friends, and more. 
Let's construct a small dataset of the last 400 tweets from the Tidyverse account. A few of the most recent tweets
are shown below.
...
```r
rt <- get_timelines('tidyverse', n=400)
```
...
with it. Note that `rtweet` can be used to manage most aspects of your account (make posts, add friends, etc.),
...
rt <- read_csv("data/tweets.csv")
rt
```
The data has quite a few variables! Let's reduce this down to a few variables of interest: `created_at`, `text`, `is_retweet`, and `retweet_screen_name`.
```{r 01-select-tweets, message = FALSE, warning = FALSE}
rt <- rt %>% select(created_at, text, is_retweet, retweet_screen_name)
rt
```
...
API---neat! From this point onward, the `rt` dataframe is stored on your
machine, and you can play with it to your heart's content; you can use
`write_csv` to save it to a file, `read_csv` to read it into R again later, and
so on. If you decide that you want to ask the Twitter API for more data 

...
Typically, in order to use the API, you have to provide the website owner with a *token*: a 
secret string of characters that is somewhat like a password.
...
(although we are not likely to get a response, and even if we did they would not likely give us permission).
...
The `read_html` function just directly downloads the source code for the page at the URL you specify---just like your browser 
would. But instead of displaying the website to you, the `read_html` function just returns the code itself, which we have

...
Learn to read in spreadsheet-like data of various formats into R from your local device and from the web. Reading data into R is the gateway to any data analysis. If you can't get it in, you can't analyze it. Additionally, there are many ways to read data into R and if you spend more time upfront matching the data reading method to the type of data you have, you will have to spend less time re-formatting, cleaning and wrangling your data (this is the second step to all data analyses). It's like making sure your shoes are tied well before going for a run so that you don't trip on the laces!
...
* list the tables in a database using the `RSQLite` library's `dbListTables` function
...
Now we will learn more about reading tabular data from a plain text file  into R, as well as how to write tabular data to a file. Last chapter we learned about using the `tidyverse` `read_csv` function when reading files that match that functions expected defaults (column names are present and commas, `,`, are used as the delimiter/separator between columns). In this section, we will learn how to read files do not satisfy the default expectations of `read_csv`. 

Before we jump into the cases where the `tidyverse` `read_csv` functions expected defaults are not the case, let's revisit how we use this with one that does and thus the only argument we need to give to the function is the path to the file, here `data/state_property_vote.csv`. We put `data/` before the name of the file when we are loading the data set because this data set is located in a sub-folder, named `data`, relative to where we are running our R code.

Here is what the file would look in plain text editor:

```{bash, echo=FALSE, comment=NA}
head -n 10 data/state_property_vote.csv
```

And here is a review of how we can use `read_csv` to load in R:
...
If the `.xlsx` file has multiple sheets, then you have to use the `sheet` argument to specify either the sheet number or name. You can also specify cell ranges using the `range` argument. This is useful in cases where the sheet contains multiple tables (a sad thing that happens to many Excel spreadsheets).
...
Another very common form of data that is read into R for the purpose of data analysis is data stored in a relational database. Common database software you might encounter include SQLite, MySQL, PosgreSQL, Oracle, and many more. All of these employ the SQL query language to pull data from the database. Thankfully several R packages have been written that allow R to connect to relational databases and use the R programming language as the front end (what the user types in) to pull data from them so that you do not have to learn the SQL query language for standard data extraction cases. In this book we will give examples of how to do this using R and the SQLite and PosgreSQL databases.
...
The first thing you need to do to read data into R from a database is to connect to the database. We do that using the `dbConnect` function from the `DBI` R package. This does not read in the data, but simply tells R where the database is.
...
We only get one table name returned form calling `dbListTables`, and this tells us that there is only one table in this data base. To reference a table in the database so we can do things like select columns and filter rows, we use the `tbl` function from the `dbplyr` package:
...
Although it looks like we just got a data frame from the database, we didn't! It is a reference, it is showing us data that is still in the SQLite database (note the first two lines of the output). It does this because often times databases are more efficient selecting, filtering and joiining large datasets than R is. And in some cases, the database will not even be on your laptop, but on a more powerful machine. So R is lazy and waits to bring this data into R's memory until you explicitly tell it to do so using the `dbplyr` `collect` function.
...
Why bother to use the `collect` function you might ask? The data looks pretty similar in both outputs shown above. Well in many cases you can directly feed the database reference (what `tbl` gives you) into downstream analysis functions (e.g., `ggplot2` for data visualization and `lm` for linear regression modeling), however, this does not work in every case (e.g., `nrow` to count rows in a data frame and `tail` to preview the last 6 rows of a data frame). Additionally, some forms of subsetting will not work to extract columns or single values from the refernce given by the `tbl` function. Thus, once you have finished your data wrangling of the `tbl` database reference object it is advisable to then bring it into memory in R using `collect` so it works as a data frame is expected to. 

### Reading data from a PostgreSQL database 

PostgeSQL (also called Postgres) is a very popular free and open-source option for relational database software. Unlike SQLite, PostgeSQL uses a client–server database engine. This is because it was designed to be used and accessed on a network, and this means that you have to provide more information to R when connecting to this kind of database software. The additional information that you need to include when you call the `dbconnect` function when connecting to a PostgeSQL database is listed below:
...
- `port` - the communication endpoint between R and the PostgeSQL database (this is typically always 5432 for PostgreSQL)
- `user` - the username for accessing the database
- `password` - the password for accessing the database


Additionally, we use the `RPostgres::Postgres` instead of `RSQLite::SQLite()` because we are now connecting to a PostgreSQL database.

Below we demonstrate how to connect to a version of the `can_mov_db` database, which contains information about Canadian movies (*note - this is a fictitional database*).
...
After opening the connection, we can again use `dbListTables` to find out what tables are in the `can_mov_db` database:
...
We see that there are 10 tables in this database. Let's first look at the `"ratings"` table and then find out what the lowest rating that exists in the `can_mov_db` database:
...
To find the lowest rating that exists in the data base, we first need to subset the `average_rating` column using `select`:
...
Instead of the minimum, we get an error! This is an example of when we need to use the `collect` function to bring the data into R so we can do further computation on it:

...
From the output above, we can see the SQL command that is sent to the database.
So instead of needing to know the SQL code ourselves, we can use `dbplyr` to do that for us. In the background, the `dbplyr` functions are translating the R code into SQL, submitting it to the database and then translating the response back for us. 

...
This subsection will show you the basics of both web scraping---using 
the [`rvest` R package](https://github.com/hadley/rvest)---and using APIs.
...
Rather than posting raw data at a URL for you to download, many data-focused websites these days
...
In particular, in this book we will show you how to use an API that obtains data 
from [Twitter](www.twitter.com). One nice feature of this particular API is that you don't need 
a special *token* to access it; you simply need to 
...
along with the examples that we show here safely. But before we go on...
...
Be careful not to overrun your quota! 

OK, back to the Twitter example. [ROpenSci](https://ropensci.org/) has created an API
called [rtweet](https://github.com/ropensci/rtweet) that lets you access data from Twitter using R.
Before getting started, make sure you review the access quotas that can be 
found [on the Twitter website](https://developer.twitter.com/en/docs/twitter-api/rate-limits).
We first load the `rtweet` package,
```
library(rtweet)
```

The first time you call a function a web page will pop up asking for your username and password.
`rtweet` requires quite extensive access to your account, so make sure you are comfortable with
that level of access before signing on.

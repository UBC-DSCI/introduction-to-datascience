...
Another very common form of data storage to be read into R for the purpose of data analysis is the relational database. There are many Relational Database Management Systems (RDBMS),  such as
SQLite, MySQL, PosgreSQL, Oracle, and many more. Almost all RDBMS employ SQL (*structured query language*) to pull data from the database. Thankfully, you don't need to know SQL
...
SQLite is probably the simplest relational database that one can use in combination with R. SQLite databases are self-contained and usually stored and accessed locally on one's computer. Data is usually stored in a file with a `.db` extension. Similar to Excel files, these are not plain text files and cannot be read in a plain text editor. 

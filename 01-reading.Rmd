...
We include a link to a short video on the difference between absolute and relative paths at the end of the chapter in the additional resources section.
...
Now that we have learned about *where* data could be, we will learn about *how* to import tabular data into R using various functions. Specifically, we will learn about reading tabular data from a plain text file (a document containing only text) into R and how to write tabular data to a file. The function we use will depend on the file's format. For example, the last chapter, we learned about using the `tidyverse` `read_csv` function when reading .csv files that match the expected defaults 
(column names are present, and commas are used as the separator between columns). In this section, we will learn how to read 
...
If the `.xlsx` file has multiple sheets, you have to use the `sheet` argument to specify the sheet number or name. You can also specify cell ranges using the `range` argument. This functionality is useful when a single sheet contains multiple tables (a sad thing that happens to many Excel spreadsheets because multiple tables makes reading in data more difficult).

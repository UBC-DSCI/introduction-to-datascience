...
concludes with an example using linear regression when there is more than one
predictor.
...
linear regression involving multiple predictor variables.
...
Let's return to the Sacramento housing data \index{Sacramento real estate} from the Chapter \@ref(regression1) to learn
...
> An additional difference that you will notice below is that we do not standardize \index{standardization}
...
will underfit \index{underfitting!regression} (have high bias), meaning that model/predicted values does not
...
case of only one predictor to the case with multiple predictors. \index{linear regression!regression with multiple predictors}\index{linear regression!regression with multiple predictors equation|see{plane equation}}
...
as in the settings of KNN regression and classification that had many predictors: more predictors is **not** always
...
We will demonstrate linear regression with many predictors using the Sacramento real estate  \index{Sacramento real estate}
...
This model is more interpretable than the comparable KNN
...
question how well linear regression with many predictors is doing compared to
the other tools we have, such as linear regression with one predictor
and KNN regression with many predictors. If this comparison is part of 
the model tuning process&mdash;for example, if we are trying
out many different sets of predictors for linear
...
For the linear regression model with two predictors, we obtain an RMSPE \index{RMSPE} 
...
is less than the prediction error for the KNN regression model that also had two predictors,
...
What can go wrong when performing linear regression? 
...
The second, and much more subtle, issue can occur when performing
linear regression with many predictors.  In particular, if you include multiple predictors that are \index{colinear}\index{multicolinear|see{colinear}}
...
If we again fit the linear regression model with two predictors on this data, then the plane of best fit
...
Therefore, when performing linear regression with many predictors, it is important to avoid including very

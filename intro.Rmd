...
In this chapter, \index{Canadian languages!overview} we will walk through a full analysis of a data set relating to
...
The data set we will study in this chapter is taken from \index{Canadian languages!canlang package}
...
the analysis as well as the selection of appropriate tools.\index{question!types and definitions}
...
6. **Estimation:**  \index{estimation!overview} taking measurements for small number of items from a large group 
...
```{r img-spreadsheet-vs-dataframe, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "A spreadsheet versus a data frame in R", out.width="850pt", fig.retina = 2}
...
frame is the *comma-separated values* format (`.csv` for short)\index{comma-separated values|see{csv}}.  These files
...
into R is called `read_csv`. \index{csv!reading} In its most basic 
...
\index{leftarrow@\texttt{<-}|see{assignment symbol}}\index{assignment symbol}
...
values from a data frame. The first argument to `filter` is the name of the data frame
...
values of a particular column. We need to pass the data frame as the first
...
We will make a bar plot to visualize our data. A bar plot \index{plot!bar} is a chart where the
...
```{r img-ggplot, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Creating a bar plot with the ggplot function", out.width="1100pt", fig.retina = 2}

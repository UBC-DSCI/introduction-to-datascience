...
name <- "Alice"
```
Note that&mdash;unlike the file name `"data/can_lang.csv"` that we specified before&mdash;when 
we name something in R using the assignment symbol, `<-`, 
we do not need to surround the name we are giving 
with quotes. This is because we are formally telling R that this special word denotes
...
symbol&mdash;e.g., `"Alice"` above&mdash;need to be surrounded by quotes.
...
answer our question, *which ten Indigenous languages were least often reported in 2016 as mother
tongues in Canada, and how many people speak each of them?* To
answer this question, we can construct a table with the ten Indigenous
languages that have the smallest counts in the `mother_tongue` column. 

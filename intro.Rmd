...
*Which ten Indigenous languages were most often reported in 2016 as mother
...
Indigenous languages is an example of a *descriptive question*: we are
...
us the ten most common Indigenous languages in Canada and their associated counts,
...
find the ten Indigenous languages that were most often reported
in 2016 as mother tongues in Canada. In particular, we will construct 
a table with the ten Indigenous languages that have the largest 
...
Indigenous languages in the data set, and then use `select` to obtain only the
...
is a string *value* and not one of the special words that make up R
...
hash symbol `#`. When R sees a `#` sign, it will ignore all of the text that

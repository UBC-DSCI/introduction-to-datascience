...
executed in a linear order, from top to bottom of the notebook, the flexibility
of the Jupyter notebook can sometimes allow us to misstep in doing this if we 
rely solely on using the **Run** ▶ button to execute cells. This can happen 
because when R named objects (e.g., a data frame named `my_data`) are defined 
when a cell is run, they continue to exist for the entire session the notebook 
is open, unless the Jupyter notebook R session (referred to in Jupyter as a 
kernel) is restarted. So objects that were created in a cell below, can be 
...
existed for that one particular R session. 

<!-- add diagram/image of cells being run out of order, and code working if the 
cells are run in the particular weird order -->

These events do not affect the user 
of that current R session, but will negatively affect others that try to run 
...
letting a user know this has happened so they can fix the issue ideally within

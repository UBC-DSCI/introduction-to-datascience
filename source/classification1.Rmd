...
`"Malignant"` and `"B"` to `"Benign"` using the `replace` method. The `replace` method \index{replace}
is used to replace values satisfying a logical condition in a data frame with other values,
and has three arguments: the original data frame column to modify, 
the logical statement to check, and the new value to use.
Furthermore, since we will be working with `Class` as a categorical
variable, it is a good idea to convert it to a factor type using the `as_factor` function. \index{factor!as\_factor}
using the astype method on the cancer data frame. 
```{r 05-class}
cancer <- cancer |>
  mutate(Class = replace(Class, Class == "M", "Malignant")) |>
  mutate(Class = replace(Class, Class == "B", "Benign")) |>
...
that we only have two different values in our `Class` column (Benign and Malignant), we 
expect to get only two names back.  Note that the `levels` function requires a *vector* argument; 

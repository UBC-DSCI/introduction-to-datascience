...
Let's visualize the clusters first as a colored scatter plot! In 
order to do that, we need to augment our 
original data frame with the cluster assignments, which we 
can extract from the fit using the `extract_cluster_assignment` function.

```{r 10-kmeans-extract-clusterasgn}
extract_cluster_assignment(kmeans_fit)
```
The output of the `extract_cluster_assignment` function is a data frame,
so we can augment the `penguins` data frame with cluster assignments
by attaching the two data frames together using the `bind_cols` function.

```{r 10-kmeans-extract-augment}
clustered_data <- bind_cols(penguins, extract_cluster_assignment(kmeans_fit))
clustered_data
```

...
- Standardize variables in R using `tidymodels`, and explain why standardization is useful in predictive modelling
- Handle imbalanced data in R using `tidymodels`, and explain why imbalanced data are problematic for classifiers
...
provides tools to help make and use models, such as classifiers.  Using this package
...
Looking at the unscaled / unshifted data above, you can see that the difference
...
We will use the `preProcess` function from `caret` to create an object we can
use to standardize the `unscaled_cancer` data set. When we do this, we need to
specify the `method` argument to be `c("center", "scale")` so that the
standardization results in the predictors having a mean (center) of 0 and a
standard deviation (scale) of 1. Finally, we use the `predict` function to
apply the data transformation to our data set:
...
<center>
```{r 05-scaling-2, echo = FALSE, fig.height = 4, fig.width = 10}

...
In the `tidymodels` framework, all data preprocessing happens using a `recipe`.
...
```{r 05-scaling-4}
uc_prepped <- prep(uc_recipe, training = unscaled_cancer)
scaled_cancer <- bake(uc_prepped, unscaled_cancer)
head(scaled_cancer)
```
...
rare_plot <- 
  ggplot() + 
    ...
    labs(color = "Diagnosis") + 
    scale_color_manual(labels = c("Malignant","Benign"), values = c(cbPalette[2], cbPalette[1]))
...
voting power in the K-nearest neighbour algorithm. In order to do this, we will modify the
earlier `uc_recipe` recipe to include the step of oversampling based on the `Class` column.
...
ups_prepped <- prep(ups_recipe, training = rare_cancer)
upsampled_cancer <- bake(ups_prepped, rare_cancer)

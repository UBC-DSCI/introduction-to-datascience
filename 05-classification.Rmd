...
focus on the widely used **K-nearest neighbours** algorithm for the
...
we define our own colourblind friendly palette here (`cbPalette`) and pass it as the `values` argument to
...
cbPalette <- c("orange2", "steelblue2")
...
observations typically fall in lower left-hand corner of the plot. In other words, benign observations tend to have lower concavity and perimeter values and malignant ones tends to have larger concavity and perimeter values. Suppose we
...
that observation as benign or malignant? Based on the scatterplot, how might you classify that new observation? How would you classify a new observation with
...
> This formula -- sometimes called the *Euclidean distance* -- is the straight line distance between two points on the x-y plane with coordinates $(x_a, y_a)$ and $(x_b, y_b)$.
...
(low `alpha` value) and large point radius. We include the code here as an optional learning challenge. It is not expected that you understand this code, but for those who want a challenge, see

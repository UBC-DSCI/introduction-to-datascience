...
and how to use our observed data to make predictions. The next will focus on how to evaluate the predictive accuracy of our classifier, as well as how to 
choose the parameters of the classifier in order to maximize its accuracy.

## Chapter learning objectives 
- Recognize situations where a classifier would be appropriate for making predictions
- Describe what a training data set is and how it is used in classification
- Identify when it is necessary to center and scale variables before classification and do this using R
...
enables us to easily manipulate factors in R. Factors are a special categorical type of variable in R that are very helpful when doing statistical inference and 
machine learning with categorical variables.
...
Before we start doing any modelling, let's explore our dataset. Below we use the `group_by` + `summarize` code pattern we used before to see that we have 357 (63\%) benign and 212 (37\%) malignant tumour observations.
...
<center>
```{r 05-multiknn-1, echo = F, fig.height = 4, fig.width = 5}
perim_concav <- cancer %>%    
  ggplot(aes(x=Perimeter, y=Concavity, color = Class)) + 
    geom_point() +
...
are multiple classes and more than two variables. Thankfully, in R, the K-nearest neighbour algorithm is implemented in the [`caret` package](https://topepo.github.io/caret/index.html) (**c**lassification 
**a**nd **re**gression **t**raining). The `caret` library contains a set of tools to help make and use predictive models, such as classifiers. 
...
We will use the function `train()`, where the argument `x` is a data frame object containing the explanatory variables/predictors, and `y` is a numeric or factor vector containing 
the outcomes/labels/classes. The arguments `x` and `y` should come from the original data frame and be in the same order. The argument `tuneGrid` should be a data frame with 
possible "tuning values". For now, just know that this is where we will specify $K=5$ (the number of nearest neighbours to vote on the predicted class). We will discuss how to 
choose $K$ in the next chapter. We will use "knn" as our `method`. 
...
```
> **Note:** the `caret` package expects `data.frames` and not `tibbles` (which are special kind data frames). This is a 

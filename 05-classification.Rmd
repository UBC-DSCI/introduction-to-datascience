...
> **`preProcess`ing Dataframes:** When we apply the `preProcess` function to classification data, we can simply
> pass in the whole dataframe because `preProcess` knows to ignore `<fct>`-type variables. But in later chapters
> in this text, we will be predicting numerical `<dbl>`-type variables; in this case, you must separate
> your predictors and outputs prior to `preProcess`ing.

...
will need a classification algorithm. Here we will use the K-nearest neighbours
...
is a number that we must choose; for now, we will assume that someone has chosen
...
diagnosis "Class" is unknown. This new observation is the red scatter point in
...
concavity of `r new_point[2]`. Looking at the scatterplot in Figure \@ref(fig:05-knn-4), how would you
...
The straight-line distance between any pair of points $(x_a, y_a)$ 
and $(x_b, y_b)$ on the x-y plane can be computed using the following
formula: 
...
to each observation in our training data, and take the $K$ observations corresponding to the
$K$ *smallest* distance values. For example, suppose we want to classify a new 
observation with perimeter of `r new_point[1]` and 
concavity of `r new_point[2]`, shown in \@ref(fig:05-multiknn-1). Let's calculate the distances
between our new point and each of the observations in the training set to find
the $K=5$ training observations that are nearest to our new point. 
You will see in the `mutate` step below, we compute the straight-line
distance using the formula: we square the differences between the two observations' perimeter 
...
Below we show in mathematical detail how the `mutate` step was used to compute the `dist_from_new` 
...
$$\mathrm{Distance} = \sqrt{(u_{1} -v_{1})^2 + (u_{2} - v_{2})^2 + \dots + (u_{m} - v_{m})^2}.$$
...
```{r 05-more, echo = FALSE, fig.cap = "3D scatterplot of the symmetry, concavity, and perimeter variables."}
library(plotly)
cancer %>%
  plot_ly(
    x = ~Perimeter,
...
a single recipe step; in this book we will use the separate `step_scale` and `step_center` steps. 

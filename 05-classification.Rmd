...
```{r download unscaled data and clean up, warning = FALSE, echo =  FALSE, message = FALSE}
...
we will use the `preProcess` function in from `caret` to create an object that we can use to transform the unscaled data to scaled data. To tell this function how we would like to do this we need to specify the `method` argument to be `c("center", "scale")`. Next we use the `predict` function to apply the data transformation to our data set:

...
> You might also notice that in the `n = 50` case and larger, that skew seems disappear.
...
construct **confidence intervals** using just a single sample from a population. A 
...
```{r 11-intro-bootstrap-image, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Overview of the bootstrap process", fig.retina = 2, out.width="880pt"}
...
listings in Vancouver, Canada, using a single sample size 40.
...
sample mean, which is centred at the population parameter value). 

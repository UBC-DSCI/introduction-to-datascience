...
classify this red, triangular observation? The nearest neighbor to this new point is a
...
are malignant. Therefore we take majority vote and classify our new red, triangular
...
more dimensions. Let's say we have three predictor
variables, perimeter, concavity, and symmetry (so, a 3-dimensional space) for two observations where
$\mathrm{observation}_1 = (0.417, 2.31, 0.837)$ and $\mathrm{observation}_{new} = (0, 3.5, 1)$.
...
Let's calculate the distances between our new observation (with a perimeter of 0, concavity of 3.5 and symmetry of 1) and each of the observations in the training set to find the $K=5$ neighbors when we have three predictors: perimeter, concavity and symmetry. 
```{r}
new_obs_Perimeter <- 0
new_obs_Concavity <- 3.5
new_obs_Symmetry <- 1
...
cancer |>
  ...
  slice(1:5) # subset the first 5 rows
```
Based on $K=5$ nearest neighbors with these three predictors we would classify the new observation as malignant. 
...
# plot the scatter
unscaled <- ggplot(unscaled_cancer, aes(x = Area, y = Smoothness, group = Class, color = Class, shape = Class, size = Class)) +
  geom_point(alpha = 0.6) + 
  scale_color_manual(name = "Diagnosis", 
                     labels = c("Benign", "Malignant", "Unknown"), 
                     values = c("steelblue2", "orange2", "red")) +
...
# plot the scatter
scaled <- ggplot(scaled_cancer, aes(x = Area, y = Smoothness, 
                                    group = Class, color = Class, shape = Class, size = Class)) +
  geom_point(alpha = 0.6) + 
  scale_color_manual(name = "Diagnosis", 
                     labels = c("Benign", "Malignant", "Unknown"), 
                     values = c("steelblue2", "orange2", "red")) +
...
rare_plot <- bind_rows(rare_cancer, tibble(Perimeter = new_point[1], Concavity = new_point[2], Class = "unknown")) |>

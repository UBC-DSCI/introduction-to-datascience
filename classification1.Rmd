...
```{r 05-setup, echo = FALSE, message = FALSE, warning = FALSE}
...
focus on the widely used **K-nearest neighbors** algorithm.
...
we want to answer. Here, the question is *predictive*: can we use the tumor
...
that we only have 2 different values in our `Class` column (B for benign and M 
...
of benign and maligant tumor observations in our data set. The `n` function within
the `summarize` function counts the number of observations in each `Class` group. 
We have 357 (63\%) benign and 212 (37\%) malignant tumor observations.
...
we define our own colourblind-friendly colors using&mdash;`"orange2"` 
...
perim_concav <- cancer %>%
  ...
  labs(color = "Diagnosis", x = "Perimeter (standardized)", y = "Concavity (standardized)") +
...
observations typically fall in lower left-hand corner of the plot. In other words, benign observations 
tend to have lower concavity and perimeter values, and malignant ones tends to have larger 
concavity and perimeter values. Suppose we
...
for the tumor class). We could compute the perimeter and concavity values,
...
classify this red observation? The nearest neighbor to this new point is a
...
neighboring points, say $K = 3$, that are closest to the new red observation
...
<!-- For our red observation at (`r new_point`), the nearest points are: (`r round(neighbors[1, c(attrs[1], attrs[2])], 1)`), (`r round(neighbors[2,  c(attrs[1], attrs[2])],1)`), and (`r round(neighbors[3, c(attrs[1], attrs[2])],1)`). -->
...
perim_concav <- cancer |>
  ggplot(aes(x = Perimeter, y = Concavity, color = Class)) +
  geom_point(alpha = 0.5) +
  scale_x_continuous(name = "Perimeter (standardized)", breaks = seq(-2, 4, 1)) +
  scale_y_continuous(name = "Concavity (standardized)", breaks = seq(-2, 4, 1)) +
...
variable (the distance to the new observation) for each of the 5 nearest neighbors in the training
data.
...
k <- 5
tab <- data.frame(neighbors[1:k, ], cancer[order(my_distances$Distance), ][1:k, c("ID", "Class")])
...
```{r 05-multiknn-3, echo = FALSE, fig.cap="Scatter plot of concavity versus perimeter with 5 nearest neighbors circled"}
...
Let's say we have three predictor
variables, perimeter, concavity, and symmetry (so, a 3-dimensional space) for two observations where
$observation_1 = (0.417, 2.31, 0.837)$ and $observation_2 = (0, 3.5, 1)$.
...
Let's calculate the distances between our new observation and each of the observations in the training set to find
the $K=5$ neighbors when we have three predictors: perimeter, concavity and symmetry. 
```{r}
new_obs_Perimeter <- 0
new_obs_Concavity <- 3.5
new_obs_Symmetry <- 1
...
Figure \@ref(fig:05-more) shows what
...
new_obs_3 <- tibble(Perimeter = 0, Concavity = 3.5, Symmetry = 1, Class = "unknown")
my_distances_3 <- table_with_distances(cancer[, attrs], new_obs_3[, attrs])
...
scaled_cancer_3 |>
  plot_ly(
    ...
    symbol = ~Class, symbols = c('circle','circle','x')
  ) |>
  add_markers() |>
  layout(scene = list(
    xaxis = list(title = "Perimeter (standardized)"),
    yaxis = list(title = "Concavity (standardized)"),
    zaxis = list(title = "Symmetry (standardized)")
  ))
```
...
perimeter 0, concavity 3.5, and an unknown diagnosis label. Let's pick out our 2 desired
...
which weight each neighbor's  vote differently, can be found on 
...
Is this predicted label the true label? We don't know, because we do not have the 
label&mdash this is what we were trying to predict. In the next chapter we will 
...
our variables' mean and standard deviation (a number measuring how spread out values are). 
...
neighbor algorithm, we will read in the original, unscaled Wisconsin breast
...
Looking at the unscaled and uncentered data above, you can see that the difference
...
Here we are calculated the required statistics based off the data input into the recipe, but
the data are not yet scaled and centered. In order to actually scale and center the data 
we need to apply the `bake` function to the unscaled data. 
```{r 05-scaling-4}
...
say, new data that you want to predict, which were not part of the training set. 
...
`Smoothness` variables ourselves before building our KNN model? Well,
...
neighbors is dominated by the much larger-scale area variable. On the right,
...
in an important way when we are using predictive algorithms. Standardizing your data should be a part of the preprocessing you do
before predictive modelling and you should think your problem domain and whether you need to standardize your data. 
...
library(ggforce)
ggplot(unscaled_cancer, aes(x = Area, y = Smoothness, group = Class, color = Class, shape = Class)) +
...
of the benign observations. We choose these 3 observations using the `slice_sample`
function, which takes two arguments: a data frame-like object to sample from 
and the number of rows to be randomly selected (`n`).
...
rare_cancer <- bind_rows(
    ...
    cancer |> 
      filter(Class == "M") |> slice_sample(n = 3)
    ) |>
  select(Class, Perimeter, Concavity)
...
> This is because we are using `slice_sample` to artificially pick only 3 of
> the malignant tumor observations, which uses random sampling to choose which
> rows will be in the training set. In order to make the code reproducible, we
> use `set.seed` to specify where the random number generator starts for this
...
As an example, we will predict the class label of a two new observations:

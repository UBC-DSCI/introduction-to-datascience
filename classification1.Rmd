...
perim_concav <- cancer %>%
  ...
  labs(color = "Diagnosis", x = "Perimeter (scaled)", y = "Concavity (scaled)") +
...
perim_concav <- cancer |>
  ggplot(aes(x = Perimeter, y = Concavity, color = Class)) +
  geom_point(alpha = 0.5) +
  scale_x_continuous(name = "Perimeter (scaled)", breaks = seq(-2, 4, 1)) +
  scale_y_continuous(name = "Concavity (scaled)", breaks = seq(-2, 4, 1)) +
...
variables, perimeter, concavity, and symmetry (so, a 3-dimensional space) for two observations.
...
three variables.  In other words, the distance formula becomes
...
scaled_cancer_3 |>
  ...
  add_markers() |>
  layout(scene = list(
    xaxis = list(title = "Perimeter (scaled)"),
    yaxis = list(title = "Concavity (scaled)"),
    zaxis = list(title = "Symmetry (scaled)")
  )) 
...
We can extend this formula beyond three predictors. Suppose we have $m$ predictor
...
We finally use the `bake` function to apply the recipe to the unscaled data.
```{r 05-scaling-4}
...
scaled <- ggplot(scaled_cancer, aes(x = Area, y = Smoothness, group = Class, color = Class, shape = Class)) +
  ...
  labs(x = "Area (scaled)", y = "Smoothness (scaled)") + 
  # coord_equal(ratio = 1) +
  geom_segment(aes(
...
each area of the plot to the decision the K-nearest neighbor 

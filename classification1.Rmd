...
manipulate factors in R. As we learned in Chapter \@ref(viz), factors are a special categorical type of variable in
...
exactly the same K-nearest neighbours algorithm applies when you
...
the K-nearest neighbours algorithm is implemented in the `parsnip` package 
included in the 
...
Let's walk through how to use `tidymodels` to perform K-nearest neighbours classification. 
We will use the cancer data set from above, with
perimeter and concavity as predictors and $K = 5$ neighbours to build our classifier. Then
...
```{r 05-scaling-plt-zoomed, echo = FALSE, fig.cap = "Zoomed in plot of unstandardized data"}
library(ggforce)
ggplot(unscaled_cancer, aes(x = Area, y = Smoothness, group = Class, color = Class, shape = Class)) +
  ...
  theme_bw() + 
 facet_zoom(ylim = c(0.075, 0.150), xlim= c(399.5, 402)) 
```
...
trained K-nearest neighbour model will make on a large range of new observations.

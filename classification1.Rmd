...
focus on the simple, widely used **K-nearest neighbors** algorithm.
...
we define our own colourblind-friendly palette using two colours&mdash;`"orange2"` 
...
In particular, let's say we have three predictor
...
We can extend the distance formula beyond three predictors. Suppose we have $m$ predictor
...
`cancer`; except now we will remove many of the observations of malignant tumours, simulating
...
> the malignant tumour observations, which uses random sampling to choose which

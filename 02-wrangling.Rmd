...
Let's suppose we want to look at only the rows for English in Vancouver. We are looking for the rows, where `region == "Vancouver"` AND `language == "English"`. Here we want to filter the data set to find rows that satisfy multiple conditions simultaneously. We can do this with the `&` logical operator, as we show below.
...
Suppose we were interested in the rows for only the Albertan cities (Edmonton and Calgary) in our data set. We can't use `&` as we did above because `region` cannot be both Edmonton and Calgary at once. Instead, we can use the `|` logical operator, which gives us the cases where one condition OR another condition is satisfied. In the code below, we ask R to return the rows where the `region` columns are equal to "Calgary" OR "Edmonton".

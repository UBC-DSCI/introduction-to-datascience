...
To define a **data frame**, first, let's explain a few commonly used terms in statistics. In statistics, **observations** are the things we measure (e.g. students) and **variables** are the characteristics of those observations, which *vary* (e.g. students' grades). From a data perspective, a data frame is a rectangle where the rows are the observations:
...
| numeric | Whole numbers and numbers with decimal values.  Numeric is an umbrella term that includes integer and double types |   1.23, 20 |  
...
>  Most of the time when working with numbers in R they will be double. That is the default when you create a vector using `c()` from numbers in R (even whole numbers), and when you read in whole numbers via `read_csv`.
...
We want to tidy our data because that format maps onto 
the statistical questions we are asking, 
and because most of the tools for data analysis in R have been designed 
to work most effectively (and efficiently) with tidy data.

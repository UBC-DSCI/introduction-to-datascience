...
In R, **vectors** are objects that can contain one or more elements. The vector elements are ordered, and they must all be of the same *data type*. R has a number of different basic data types as shown in table \@ref(tab:datatype-table).
...
| list | A set of values, which can have different data types. Lists can be used to represent combinations of data values.| 
...
In chapter 1, we learned about two `tidyverse` functions: `select` and `filter` to select a single column and filter a single row from a data frame. We will now learn more ways we can use these functions. Recall, the `select` function creates a subset of the *columns* of a data frame, while the `filter` creates a subset of the *rows* with specific values.
...
If we are interested in finding information about the languages who have a higher number of people who primarily speak it as their mother tongue compared to Mandarin---which is reported to have `r format(mandarin_mother_tongue, scientific = FALSE, big.mark = ",")` people speaking it as their mother tongue---then we can create a filter to obtain rows where the value of `mother_tongue` is greater than `r format(mandarin_mother_tongue, scientific = FALSE, big.mark = ",")`. In this case, we see that `filter` returns a data frame with 2 rows; this indicates that there are two languages that are spoken more often as their mother tongue compared to Mandarin.
...
Here we used a new data frame name (`mutated_can_lang`) and column name (`mother_tongue_proportion`) to create a new data frame and a new column. But we could have used the current data frame name and column name, which would have written over our original data frame and column.

...
Data is often stored in a wider, not tidy, format because this format is often more intuitive for human readability and understanding, and humans create data sets. We will work with a subset of the `region_lang` data, which contains data retrieved from the 2016 Canadian census. Our data set contains the counts for each census division of how many Canadians report each language as their mother tongue for five major Canadian cities (Toronto, Montreal, Vancouver, Calgary and Edmonton).  Our data set is stored in an untidy format as shown below:

```{r 02-tidyverse, warning=FALSE, message=FALSE}
library(tidyverse)
region_lang_wide <- read_csv("data/region_lang_top5_cities_wide.csv") 
region_lang_wide
```
...
To accomplish this data tranformation we will use the `tidyverse` function `pivot_longer`. To use gather we need to specify the:
...
```{r}
region_lang_tidy <- pivot_longer(region_lang_wide, 
...
Suppose we have the opposite problem we had above and we need to separate observation currently stored in a single column into multiple columns. To do this we can use the function `pivot_wider`. `pivot_wider` acts to increase the number of columns, and thus makes the data frame wider.

For example, each observation is a census division, but each observation is is two rows, as shown in the example below:
```{r 02-reading-long, warning=FALSE, message=FALSE}
region_lang_long <- read_csv("data/region_lang_top5_cities_narrow.csv") 
region_lang_long
```
...
the Toronto, Montreal, Vancouver, Calgary and Edmonton columns contain both the counts of Canadians that speak each language as their mother tongue and the population of that region. To make this messy data tidy we'll have to fix both of these issues.

```{r, warning=FALSE, message=FALSE}
region_lang_messy <- read_csv("data/region_lang_top5_cities_messy.csv")
region_lang_messy 
```

First we’ll use gather to create two columns: region and value, similar to what we did previously:

```{r}
region_lang_long_messy <- pivot_longer(region_lang_messy, 
                         cols = Toronto:Edmonton,
                         names_to = "region", 
                         values_to = "value")
region_lang_long_messy
```

Then we'll use `separate` to split the `value` column into two columns, one that contains only the counts of Canadians that speak each language most at home, and one that contains the counts for most at work for each census division:

```{r}
region_lang_pop_tidy <- separate(region_lang_long_messy,
                                 col = value, 
                                 into = c("most_at_home", "most_at_work"), 
                                 sep = "/") 
region_lang_pop_tidy <- mutate(region_lang_pop_tidy,  
...
For example, we could visualize the two most common languages reported by Canadians as their primary language at home for each of these five major Canadian cities:

```{r 02-plot}
top10 <- region_lang_pop_tidy %>% 
  arrange(desc(most_at_home)) %>% 
  group_by(region) %>% 
  slice(1:2)

ggplot(top10, aes(x = region, y = most_at_home, fill = language)) + 

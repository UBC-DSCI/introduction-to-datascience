...
To create the vector of years in Figure \@ref(fig:02-vector) we write:
```{r}
...
First, one of R's most popular plotting toolsets, the `ggplot2` package (which is one of the packages that the `tidyverse` package loads), expects the data to be in a tidy format. Second, most statistical analysis functions also expect data in a tidy format. Given that both of these tasks are central in almost all data analysis projects, it is well worth spending the time to get your data into a tidy format upfront. Luckily there are many well-designed `tidyverse` data cleaning/wrangling tools to help you easily tidy your data. Let's explore them below!
...
values, so if we wanted to find the maximum year for example, it's harder to do 
when the values are not in their own column. We can reshape this data set to a long format 
by creating a column called "year" and a column called "population," which is the table on the right.
...
For example, in Figure \@ref(fig:long-to-wide), the table on the left is an
untidy format because the `count` column contains two variables (population and commuter count). 
Using data in this format means we can't easily access the count values,
so if we wanted to find the maximum population, it's harder to do when the values are not in their own column. We can reshape this data set to a wider format by creating a column called "population" and a column called "commuter," which is the table on the right.
...
### Using `mutate` to create and modify columns
...
In this case, we see that `filter` returns a data frame with only one row; this indicates that only one English in Toronto is reported by more people as their primary language at home than French in Montréal.
...
Then the second element of `vector1` is then compared to all the elements in `vector2` etc. 
...
Below we use `mutate` to calculate
the proportion of people reporting a given language as their primary language at home and work for all the languages in the `official_langs` data set. 
```{r}
...
Is this data set, `offical_langs`, now tidy? Well, if we recall the three criteria for tidy data:
...
> Note `%>%` is the old pipe feature so you may see it used as the pipe operator
in some textbooks or online sources. In this textbook we will be using the new pipe operator `|>`. 

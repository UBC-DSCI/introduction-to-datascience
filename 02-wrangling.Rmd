...
Below we use `mutate` to convert the columns `most_at_home` and `most_at_home` to numeric data types in the `official_langs` data set as described in figure \@ref(fig:img-mutate):
...
In this case, we see that `filter` returns a data frame with only one row; this indicates that only one language in a region is spoken more often at home than French in Montréal.

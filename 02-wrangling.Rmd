...
> `vector1 == vector2` 
will compare the vectors element by element. It checks if the first element of 
...
```{r 02-plot}
ggplot(official_langs, aes(x = region, y = most_at_home_proportion, fill = language)) +
  geom_bar(stat = "identity") +
  scale_color_manual(values = c("deepskyblue2", "firebrick1")) +
  xlab("Region") +
  scale_y_continuous(
    name = "Proportion of residents reporting their primary language at home",
    labels = scales::comma
  ) +
...
> Note `%>%` is the old pipe operator so you may see it used 

...
region_lang_messy 
```
We'll use `separate` to split the language column into two columns, one that contains only the language, and one that contains a the category for which that language falls into:

```{r}
region_lang_tidy <- separate(region_lang_messy,
                                 col = language, 
                                 into = c("category", "language"), 
                                 sep = "/") 
region_lang_tidy
```

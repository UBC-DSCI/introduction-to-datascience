...
Messages act as a very useful to narrative 
...
```{r vc2-changes, fig.cap = 'Local repository with changes to files.', fig.retina = 2}
...
```{r vc-ba2-add, fig.cap = 'Adding modified files to the staging area in the local repository.', fig.retina = 2}
...
```{r vc-ba3-commit, fig.cap = "Committing the modified files in the staging area to the local repository history, with an informative message about what changed.", fig.retina = 2}
...
you need to **push** (i.e., send) those commits back to GitHub (Figure \@ref(fig:vc5-push)). This updates 
...
```{r vc5-push, fig.cap = 'Pushing the commit to send the changes to the remote repository on GitHub.', fig.retina = 2}
...
```{r vc7-pull, fig.cap = 'Pulling changes from the remote GitHub repository to synchronize your local repository.', fig.retina = 2}
...
In particular, we will interact with Git using the Jupyter Git extension tool,
and use GitHub to create and manage the remote repository.

There are several different workflows you can employ to start using version control
with a new project. For simplicity and ease of setup, 
we recommend creating a remote repository on GitHub first, 
and then **cloning** (or copying) that repository to create a local repository.
...
To get started with a *public* repository having a template `READEME.md` file, take the 
following steps shown in Figure \@ref(fig:new-repository-02): 

1. Enter the name for your project repository. In the example below, we use `canadian_languages`. Most repositories follow a similar naming convention involving only lowercase letter words separated by either underscores or hyphens.

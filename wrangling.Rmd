...
Given that most of the tools we have used to import data represent the data 
as a *data frame* in R, 
we will spend some time learning more about data frames 
to deepen our understanding of them. 
This will be useful prerequisite knowledge to use
...
object whose columns are *vectors*. 
For example, the data frame in Figure \@ref(fig:02-vectors) 
has three vectors whose names are `region`,
`year` and `population`. We will explain what vectors are in the next section.

``` {r 02-vectors, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Data frame with three vectors", fig.retina = 2, out.width = "700pt"}
...
``` {r 02-vector, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Example of a vector whose type is character.", fig.retina = 2, out.width = "700pt"}
...
| logical | lgl | A value of true and false | `TRUE`, `FALSE` |
| factor | fct | Used to represent data with a limited number of values | a `color` variable with levels `red`, `green` and `orange` |
...
> numbers using `c()`, and when you read in whole numbers via `read_csv`. 
> Factors help us represent categorical data. Factors can be ordered or 
> unordered, and even though factors look like characters, they are integers
> underneath. They contain a set of values, known as levels. 
> Storing something as a factor helps ensure that R will treat the data correctly.
> There are other basic data types in R, such as *raw* and *complex*, which we
> won't cover in this textbook.
...
A data frame is really a special subtype of list where the elements 
must be either a vectors or a list, 
and these elements must be of the same length. 
...
but because the columns in this example are vectors, 
the elements must be the same data type within each column. 
Representing data with data frames enables us to work with our 
data in a tabular manner easily. 
It also allows us to have columns (containing qualities or values) 
for different variables linked in a row for one observation,
and these columns can't be sorted independently. 
For example, suppose you had a data frame where the rows were students 
and the columns were students' grades.
If you sorted students by their name alphabetically, 
you would want the grades associated with each student to get sorted 
along with each name rather than sorting the columns independently. 
...
> **Note:** In this book we have chosen to use the term "vector" to refer to 
> the data structure that is technically called an atomic vector 
> (e.g. character, integer, double, logical, etc).
> We have made this decision because this is how this data type 
> is most commonly referred to by the R community.
> Formally, vectors is an umbrella term that encompasses both atomic vector 
> and list objects in R.
> Confusing we know and agree!
> We encourage readers who are enthusiastic to learn more 
> to read the Vectors chapter of the Advanced R book [@wickham2019advanced].
...
"population," which is the table on the right of 
...
We can set `convert = TRUE` to convert the `most_at_home` 
...
Chapter \@ref(intro) discussed two `tidyverse` functions: 
`select` and `filter` to select columns and filter rows from a data frame. 
We will now learn more ways we can use these functions. 
Recall, the `select` function creates a subset of the *columns* of a data frame, 
while the `filter` subsets *rows* with specific values.
...
but with less typing, and thus, we are less error-prone! 
This type of operator is handy, especially for large data sets.
...
Check out the Additional Resources section at the end of the chapter 
for a comprehensive resource on `select` helpers.
...
The number of people who report that English was their primary language at home 
...
Let's now use `mutate` to create a new column in our data frame 
...
that we had to ensure that we ordered the `city_pops` vector in the
...
ggplot(
  ...
  xlab("Proportion reporting they speak English most often at home") +
  ylab("Region")
```
...
Vancouver, 
English was reported as the most common primary language used at home.
...
> **Note:** 
> In this textbook, we will be using the base R pipe operator syntax, `|>`.
...
But wait - 
why does our `select` function call look different in these two examples? 
...
and thus you do not specify that argument in that function call. 
In the code above,
the first argument of `select` is the data frame we are `select`-ing from, 
which is provided by the output of `filter`.
...
For example, you might store temporary objects 
...
### Calculating summary statistics on columns
...
```{r summarize, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "`summarize` is useful for calculating summary statistics on one, or more, column(s). In it's simplest use case, it creates a new data frame with a single row containing the summary statistic(s) for each column being summarized. The darker, top row of each table represents the column headers.", out.width="100%", fig.retina = 2}
...
### Calculating summary statistics on columns for groups of rows
...
```{r summarize-groupby, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "`summarize` + `group_by` is useful for calculating summary statistics on one, or more, column(s), for each group. It creates a new data frame with a row for each group, containing the summary statistic(s) for each column being summarized. It also creates a column listing the value of the grouping variable. The darker, top row of each table represents the column headers. The grey, blue and green coloured rows correspond to the rows that belong to each of three groups being represented in this cartoon example.", out.width="100%", fig.retina = 2}
image_read("img/summarize/summarize.002.jpeg") |> 
  image_crop("2000x475+0+300")
```

For example, 
here, we can use `group_by` to group the regions of the `tidy_lang` data frame
...
```{r summarize-across, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "`summarize` + `across` or `map` is useful for efficiently calculating summary statistics on many columns at once. The darker, top row of each table represents the column headers.", out.width="100%", fig.retina = 2}
...
This is a common task when converting units of measurement across many columns, 
and when standardizing data before modelling. 
We illustrate such a data transformation in Figure \@ref(fig:mutate-across).

```{r mutate-across, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "`mutate` and `across` is useful for applying functions across many columns. The darker, top row of each table represents the column headers.", out.width="100%", fig.retina = 2}
...
which means that we get back the same data frame (with the same number of rows).
Just the values in the mutated columns are different.
...
the only thing that changes is the tranformation we applied 
...
```{r rowwise, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "`rowwise` + `mutate` is useful for applying functions across columns within one row. The darker, top row of each table represents the column headers.", out.width="100%", fig.retina = 2}

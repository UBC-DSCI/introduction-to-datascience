...
By the end of the chapter, readers will be able to:
  ...
  - recall and use the following functions and operators for their
...
A **data frame** is a table-like structure and is a very common method for storing 
data in R. We have actually seen and worked with data frames already in 
Chapter \@ref(reading), but we will define it more precisely in this chapter. 
But first, we need to introduce a few commonly used terms in statistics:
...
all elements are integers), whereas elements within a single list can be of
different types (e.g., characters, integers, logicals and even other lists can be
elements in the same list). Lists are sometimes called recursive vectors 
...
can only be vectors. Although the columns of a data frame can only be vectors, 
...
ability to add useful (such as grouping, which we will discuss later) attributes
and more predictable type preservation when subsetting. Because a tibble is just a data frame with some
...
recorded. We can see how we would like to transform the data from long to wide with
`pivot_wider` in Figure \@ref(fig:img-pivot-wider-table). 
Suppose we wanted to visualize the relationship between the number of
Canadians reporting their primary language at home and work. It would be
difficult to do that with the messy data table on the left since these two
...
data frame object, `tidy_lang`. The second argument is a logical
statement to use when filtering the rows. Suppose we are only interested in official 
languages in this data set. We use the *equivalency operator* (`==`) to compare the values of the `category` column with the value `"Official languages"`. With these arguments, `filter` returns a
...
### Using `filter` to extract rows with `&` or `,` {#filter-and}

Let's suppose we want to look at only the rows for French in Montréal. Here we
...
ampersand sign (`&`). The filter function treats the comma (`,`) between the logical
...
five_cities <- filter(region_data, 
                      region %in% c("Toronto", "Montréal", "Vancouver", "Calgary", "Edmonton"))
five_cities
...
We saw in section \@ref(filter-and) that `r format(most_french, scientific = FALSE, big.mark = ",")` people
...
To extract the populations of our five cities we will first arrange the `five_cities`
data set that we created above according to the population size (from largest to smallest) 
using the `arrange` function. Recall this function takes column names as
input and orders the rows in the data frame in ascending order based on the
values in the columns. There we need to add `desc` in front of the `population` 
variable to order the rows in descending order. 

``` {r find-city-populations}
ordered_cities <-  arrange(five_cities, 
                           desc(population)) 
ordered_cities
```
Then we will use the `pull` function, which extracts a column in a data frame 
and transforms it into a vector. We use `pull` instead of `select` because for the 
next step we want the population values as a vector rather than a tibble, which would be the 
output of using `select`. 
```{r}
city_populations <- pull(ordered_cities, 
                         population)
city_populations
```
Then we use `mutate` to
...
(`r format(toronto_popn, scientific = FALSE, big.mark = ",")`). Likewise, we want to
divide the `most_at_home` value for French in Toronto
(`r format(toronto_fr, scientific = FALSE, big.mark = ",")`) and by
...
answer the question: what proportion of people in each city speak Canada's official 
languages as their primary language at home in these five cities? Since our data is
...
``` {r}
lang_summary <- summarize(tidy_lang,
...
``` {r}
lang_summary_by_region <- group_by(tidy_lang, region) |>
summarize(
  min_most_at_home = min(most_at_home),
...
```{r}
group_by(tidy_lang, region)
```

## Iterating over columns of a data frame

Where should you turn when you discover the next step in your data
wrangling/cleaning process requires you to apply a function to *every* column in a
data frame? The process of repeating a function, say on different columns or data sets,
is called **iteration**. For example, suppose you wanted to
know the maximum value of every column in a data frame. You could use `summarize`
...
In cases, where you want to apply the same data transformation to all
columns, it is more efficient to use `dplyr`'s `across` function with `summarize` to apply a
transformation to each column. For example, let's find the maximum value of each of the numeric
columns of the `region_lang.csv` data set. First, let’s read in and peek at the data to familiarize
ourselves with it:

``` {r 02-preview-region-data, message = FALSE}
...
The approach above gets the job done, but if we had many columns in our
...
``` {r 02-read-in-nas, message = FALSE, warning = FALSE}
...
Thus, we also need to add the argument `na.rm = TRUE` ("NA
...
An alternative to `summarize` and `across` for iteration is `purrr`'s `map` function. 
...
We will select only the numeric columns of the data frame:
...
Similarly to `summarize` and `across`, `map_df()` returns `NA` for columns that 
contain NAs in the data frame (remember that is what happens with many of the
built-in R statistical functions when NA's are present...).
``` {r 02-map-na}
map_df(region_with_nas, max)
```

We need to add the argument `na.rm = TRUE` to the end of our call to

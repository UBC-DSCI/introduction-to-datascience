...
``` {r}
lang_tidy <- separate(lang_messy_longer,
  col = value,
...
`most_at_home` and `most_at_work` from the `lang_tidy` data set. We can
use the `select` function to choose the columns that we want and assign it to a
data frame:

``` {r}
selected_columns <- select(lang_tidy, language, region, most_at_home, most_at_work)
selected_columns
```
...
the `lang_tidy` data frame from `language` to `most_at_work` we pass
`language:most_at_work` as the second argument to the `select` function.

``` {r}
column_range <- select(lang_tidy, language:most_at_work)
column_range
```
...
``` {r 02-select-helpers}
select(lang_tidy, starts_with("most"))
```
...
data frame object, `lang_tidy`. The second argument is a logical
...
``` {r}
official_langs <- filter(lang_tidy, category == "Official languages")
official_langs
```
...
``` {r}
filter(lang_tidy, category != "Official languages")
```
...
To find out the populations of each city we will first filter the `region_data`
for only the rows where region is equal to one of our desired cities. Then we arrange it 
according to the population size and use the `pull` 
function, which takes a column in a data frame and transforms it into a vector. 

``` {r find-city-populations}
city_populations <- region_data |> 

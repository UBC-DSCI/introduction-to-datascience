...
However, for R, to do any visualization or analysis this format is difficult to work with. 
...
`pivot_longer`. The table on the left in Figure \@ref(fig:img-pivot-longer-with-table) represents the data in the "wide" format and the table on the right represents how we want the data to look after we've applied the `pivot_longer` function. 
...
Figure \@ref(fig:img-pivot-longer) details what arguments we need to specify to use `pivot_longer`.
...
\@ref(pivot-wider), the `most_at_home` and `most_at_work` variables were `<dbl>` (double) (you can verify this by looking at the tables in the previous sections), which is a type of numeric data. This change is due to the delimiter "/" when we read in this messy data set. R read the columns in as character types, and by default, `separate` will return columns as character data types. 
...
There are many different `select` helpers, which are very handy for selecting variables based on certain criteria. Check out the resource in the additional resources section for a complete list of `select` helpers. 
...
The function treats the comma `,` between the logical statements as an "AND". 
...
> Let's suppose we have two vectors, `vectorA` and `vectorB`. If you type 
> `vectorA == vectorB` into R it
will compare the vectors element by element. R checks if the first element of 
`vectorA` equals the first element of `vectorB`, the second element of `vectorA`
equals the second element of `vectorB` etc. 
> On the other hand, `vectorA %in% vectorB`
compares the first element of `vectorA` to all the elements in `vectorB`. 
Then the second element of `vectorA` is then compared to all the elements in `vectorB` etc. Notice the difference between `==` and `%in%` in the example below. 
...
apply a transformation to each column. For example, let's find the maximum value of each column of the `region_lang`. First, let’s peek at the data to familiarize ourselves with it:

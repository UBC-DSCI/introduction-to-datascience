...
This knowledge will helpful in effectively utilizing these objects in our data analyses.
...
Given these definitions, a **data frame** is a table of values where the rows
are observations and the columns are variables. For example, Figure
...
Internally, R stores the columns (i.e., variables) in a data frame as either
...
R, along with many of the `tidyverse` functions we use in this book, treat 
different data types very differently. You should use integers and double types
(which both fall under the "numeric" umbrella type) to represent numbers and perform
arithmetic. Doubles are more common, though; for instance, a double data type is the
...
represent categorical data, and can take one of a set of discrete
values known as *levels*. The levels can be ordered or unordered.  Even though
...
internally stores factors using integers! It is important to use factors&mdash;not characters&mdash;to
store categorical data. There are other basic data types in R, such as *raw*
...
> You can use the function `class` \index{class} on a data object to assess whether a data
...
> **Note:** Is there only one shape for tidy data for a given data set? Not necessarily! It
...
the columns between Toronto and Edmonton:  \index{colon symbol}\index{colonsymb@\texttt{:}|see{colon symbol}}
...
colon (`:`) operator to denote the range. For example, to get all the columns in \index{colon symbol}
...
data frame for the remainder of this chapter.
...
We can also use the `&` logical operator, which gives 
us cases where *both* one condition *and* another condition
are satisfied. You can use either `,` or `&` in the `filter`
...
### Extracting rows satisfying at least one of multiple conditions using `|`
...
### Extracting rows with values above or below a threshold using `>` and `<`
...
```{r 02-plot, out.width = "70%", fig.cap = "Proportions of Canadians reporting English as the most often spoken language at home."}
...
frame `data`:  \index{pipe}\index{pipesymb@\texttt{|>}|see{pipe}}
...
> and is from the `magrittr` R package. \index{pipesymbb@\texttt{%>%}|see{pipe}}
...
But wait...Why does our `select` and `filter` function calls 
...
```{r summarize, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "`summarize` is useful for calculating summary statistics on one or more column(s). In its simplest use case, it creates a new data frame with a single row containing the summary statistic(s) for each column being summarized. The darker, top row of each table represents the column headers.", out.width="100%", fig.retina = 2}
...
(e.g., `max`, `mix`, `mean`, `sum`, etc) will return `NA` 
...
For example, we can use `group_by` to group the regions of the `tidy_lang` data frame
...
and use a colon `:` to specify a range of columns we would like  \index{colon symbol}

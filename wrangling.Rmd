...
and transforms it into a vector. We use `pull` instead of `select` because we 
want the population values in a vector rather than a tibble, which would be the 
...
Notice that `group_by` on its own doesn't change the way the data looks, but instead it 
changes how other functions, such as `summarize` work with the data, as we saw above.  

```{r}
group_by(lang_home_tidy, region)
```
...
We need to add the argument `na.rm = TRUE` to the `max` function so that we get a more useful
value than `NA` returned. We will add these additional arguments to the end of our call to the
...
only the languages in Vancouver. We also `select` specific columns so we can see 
all the columns in the data frame, but note that this step is not strictly necessary. 
```{r vancouver_filter}
...
value across *all* rows rather than the maximum value for *each* row. Therefore 
...
```
Cleaning and wrangling data can be a very time-consuming process, however, it is a critical step in any data analysis. We have explored many different functions for wrangling data into a tidy format. Table \@ref(tab:summary-functions-table) summarizes some of the key wrangling functions we learned in this chapter. 

...
a subset of 18 observations of the original data, which has already been scaled. 
We will discuss scaling for K-means in more detail later in this chapter. \index{mutate}\index{read function!read\_csv} 
...
data <- read_csv("data/toy_penguins.csv") |>
  ...
  mutate(flipper_length_scaled = as.double(scale(flipper_length_mm)), 
         bill_length_scaled = as.double(scale(bill_length_mm)))

penguin_data <- data |> select(flipper_length_scaled, bill_length_scaled)
write_csv(penguin_data, "data/penguins_scaled.csv")
```
...
penguin_data <- read_csv("data/penguins_scaled.csv")
penguin_data
```
...
```{r 10-toy-example-plot, warning = FALSE, fig.height = 4, fig.width = 4.35, fig.cap = "Subset data of penguin scaled bill length versus flipper length."}
ggplot(data, aes(x = flipper_length_scaled, y = bill_length_scaled)) +
  geom_point() +
...
```{r 10-toy-example-clustering, echo = FALSE, warning = FALSE, fig.height = 4, fig.width = 5, fig.cap = "Subset data of penguin scaled bill length versus flipper length with coloured groups"}
ggplot(data, aes(y = bill_length_scaled, 
                 x = flipper_length_scaled, colour = cluster)) +
  geom_point() +
  xlab("Flipper Length (scaled)") +
  ylab("Bill Length (scaled)") + 
...
(``r paste("flipper_length_scaled =", round(mean(clus1$flipper_length_scaled),2))`` and ``r paste("bill_length_scaled =", round(mean(clus1$bill_length_scaled),2))``) highlighted 
...
base <- ggplot(clus1) +
  geom_point(aes(y = bill_length_scaled, x = flipper_length_scaled),  col = "dodgerblue3") +
...
for (i in 1:4) {
  ...
  plt_ctr <- ggplot(penguin_data, aes(y = bill_length_scaled, x = flipper_length_scaled, colour = label)) +
    ...
    theme(legend.position = "none") +
    scale_color_manual(values= cbpalette) + 
    geom_point(data = centers, aes(y = bill_length_scaled, x = flipper_length_scaled, fill = label), size = 5, shape = 21, stroke = 2, colour = "black", fill = cbpalette)

...
where there is a *response variables* (a category label or value), 
...
However, because there is no response variable, it is not as easy to evaluate
...
principle component analysis, multidimensional scaling, and more; 
see the additional resources section at the end of the next chapter 
...
> when onehas an unlabelled data set that is too large to manually label, 
...
knitr::include_graphics("img/Gentoo_Penguin._(8671680772).jpg")
```

To learn about K-means clustering in this chapter, 
we will work with `penguin_data`, 
a subset of 18 observations of the original data, 
which has already been standardized 
(remember from Chapter \@ref(classification) that scaling is part of the standardization process). 
We will discuss scaling for K-means in more detail later in this chapter. 
\index{mutate}\index{read function!read\_csv} 

Before we get started, we will load the `tidyverse` metapackage to ensure
we have the functions we need, 
as well as set a random seed, so that our analysis will be reproducible.
As we will learn in more detail later in the chapter, 
the K-means clustering algorithm uses random numbers - 
which is why setting the seed here is important.
...
penguin_data <- data |> select(flipper_length_standardized, bill_length_standardized)
...
Next, we can create a scatter plot using this data set to see if we can use
data visualization to detect subtypes or groups in our data set.

```{r 10-toy-example-plot, warning = FALSE, fig.height = 4, fig.width = 4.35, fig.cap = "Subset data of penguin standardized bill length versus standardized flipper length."}
ggplot(data, aes(x = flipper_length_standardized, y = bill_length_standardized)) +
  geom_point() +
  xlab("Flipper Length (standardized)") +
  ylab("Bill Length (standardized)")
```

Based \index{ggplot}\index{ggplot!geom\_point} on the visualization in Figure \@ref(fig:10-toy-example-plot), we might
suspect there are a few subtypes of penguins 
within the observations we have in our data set.
We can see ~ 3 groups of observations in Figure \@ref(fig:10-toy-example-plot),
including:
...
This is interesting that we can use data visualization to uncover such patterns,
but also leads to the questions of how might we find 
these kind of grouping patterns automatically, 
and how do we pick the number of groups or subtypes? 
...
```{r 10-toy-example-clustering, echo = FALSE, warning = FALSE, fig.height = 4, fig.width = 5, fig.cap = "Subset data of penguin standardized bill length versus standardized flipper length with coloured groups"}
ggplot(data, aes(y = bill_length_standardized, 
                 x = flipper_length_standardized, colour = cluster)) +
  geom_point() +
  xlab("Flipper Length (standardized)") +
  ylab("Bill Length (standardized)") + 
...
base <- ggplot(clus1) +
  geom_point(aes(y = bill_length_standardized, x = flipper_length_standardized),  col = "dodgerblue3") +
  labs(x = "Flipper Length (standardized)", y = "Bill Length (standardized)") +
  xlim(c(
    min(clus1$flipper_length_standardized) - 0.25 * sd(clus1$flipper_length_standardized),
    max(clus1$flipper_length_standardized) + 0.25 * sd(clus1$flipper_length_standardized)
  )) +
  ylim(c(
    min(clus1$bill_length_standardized) - 0.25 * sd(clus1$bill_length_standardized),
    max(clus1$bill_length_standardized) + 0.25 * sd(clus1$bill_length_standardized)
  )) +
  geom_point(aes(y = mean(bill_length_standardized), x = mean(flipper_length_standardized), colour = "red", size = 2.5)) +
  theme(legend.position = "none") 
base
```

The second step in computing the WSSD is to add up the squared distance \index{distance!K-means} between each point in the cluster and the cluster center.
We use the straight-line / Euclidean distance formula that we learned about in the [classification chapter](\@ref(classification)).
...
mn <- clus1 |> summarize(flipper_length_standardized = mean(flipper_length_standardized), bill_length_standardized = mean(bill_length_standardized))
for (i in 1:nrow(clus1)) {
  base <- base + geom_segment(
    x = unlist(mn[1, "flipper_length_standardized"]), y = unlist(mn[1, "bill_length_standardized"]),
    xend = unlist(clus1[i, "flipper_length_standardized"]), yend = unlist(clus1[i, "bill_length_standardized"])
  )
}
base <- base + geom_point(aes(y = mean(bill_length_standardized), x = mean(flipper_length_standardized), colour = "red", size = 2.5))
base
```
...
and randomly assigning an equal number of observations 
...
plt_lbl <- ggplot(penguin_data, aes(y = bill_length_standardized, x = flipper_length_standardized, colour = label)) +
  geom_point(size = 2) +
  xlab("Flipper Length (standardized)") +
  ylab("Bill Length (standardized)") +
  theme(legend.position = "none") + 
...
And we show what the the first four iterations of K-means would look like in  
...
for (i in 1:4) {
  ...
  nclus <- nrow(centers)
  # replot with centers
  plt_ctr <- ggplot(penguin_data, aes(y = bill_length_standardized, x = flipper_length_standardized, colour = label)) +
    geom_point(size = 2) +
    xlab("Flipper Length (standardized)") +
    ylab("Bill Length (standardized)") +
    theme(legend.position = "none") +
    scale_color_manual(values= cbpalette) + 
    geom_point(data = centers, aes(y = bill_length_standardized, x = flipper_length_standardized, fill = label), size = 5, shape = 21, stroke = 2, colour = "black", fill = cbpalette)
...
the straight-line / Euclidean distance is used to measure the 
distance between observations and cluster centres. 
This decision means that the data that this algorithm can use 
must be ordered and the increments evenly spaced. 
Quantitative data (e.g., heights, weights, speed, etc) satisfies these criteria, 
however, categorical data (e.g., a variable for hair colour, 
that can hold the value blonde, brown, red, black, etc)
does not. 
...
p1 <- ggplot(assignments, aes(flipper_length_standardized, bill_length_standardized)) +
  geom_point(aes(color = .cluster, size = 3)) +
  facet_wrap(~k) +   scale_colour_manual(values = cbbPalette) +
  labs(x = "Flipper Length (standardized)", y = "Bill Length (standardized)", colour = "Cluster") +
  theme(text = element_text(size = 20), legend.position = "none") +
  geom_point(data = clusters, aes(fill = cluster), colour = "black", size = 5, shape = 21, stroke = 2) + scale_fill_manual(values = cbbPalette)
...
below using an unscaled, and unstandardized version of data set in this chapter.
...
This can work because lists are super vectors, and so they are legitimate 
...
> However, before we do this, we need to turn `rowwise`.
> To do that, we use `ungroup` 
> (predictably, this can also turn `group_by` off as well).
> 
> ```{r}
> penguin_clust_ks |>
>   ungroup() |>
...
Finally we extract the total WSSD from the `glanced` column. 
...
the more likely we are to find a lucky, or good clustering (if one exists!).
...
but there is a trade-off that doing many many clusterings 

...
we sum them together to to get the *total WSSD*.
For our example, 
this means adding up all the distances of the black lines in 
...
all_clusters_base <- data |>
  ...
  scale_color_manual(values= c("darkorange3", 
                               "dodgerblue3", 
                               "goldenrod1"))
cluster_centres <- tibble(x = c(0, 0, 0),
                          y = c(0, 0, 0))
...
for (cluster_number in seq_along(1:3)) {
  ...
  cluster_centres[cluster_number, 1] <- mean(clus$flipper_length_standardized)
  cluster_centres[cluster_number, 2] <- mean(clus$bill_length_standardized)
}

all_clusters_base <- all_clusters_base + 
  geom_point(aes(y = cluster_centres$y[1], 
                 x = cluster_centres$x[1]), 
             colour = "#F8766D", size = 3) +
  geom_point(aes(y = cluster_centres$y[2], 
                 x = cluster_centres$x[2]), 
             colour = "#F8766D", size = 3) +
  geom_point(aes(y = cluster_centres$y[3], 
                 x = cluster_centres$x[3]), 
             colour = "#F8766D", size = 3)

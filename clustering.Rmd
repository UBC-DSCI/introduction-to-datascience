...
## Clustering
Clustering \index{clustering} is a data analysis task involving separating a data set into
...
Then we use `map` \index{map} to apply the `kmeans` function to each
...
The solution is to create something called an *anonymous function*. \index{function!anonymous}
...
```{r, eval = FALSE}
map(penguin_clust_ks, function(k) kmeans(standardized_data, k))
```

The anonymous function in the above call is `function(k) kmeans(standardized_data, k)`.
This function takes a single argument (`k`) and evaluates `kmeans(standardized_data, k)`.
...
```{r 10-choose-k-part2}
penguin_clust_ks <- tibble(k = 1:9) |>
  mutate(penguin_clusts = map(k, function(ks) kmeans(standardized_data, ks)))
penguin_clust_ks
```

Next, we use `map` again to apply `glance` \index{glance} to each of the K-means
...
penguin_clust_ks <- tibble(k = 1:9) |>
  mutate(
    penguin_clusts = map(k, ~ kmeans(standardized_data, .x)),
...
penguin_clust_ks <- tibble(k = 1:9) |>
  mutate(
    penguin_clusts = map(k, ~ kmeans(standardized_data, nstart = 10, .x)),
    glanced = map(penguin_clusts, glance)
  )

...
to specify the number of samples to take, here 15,000. We will use the function `head()` to see the first few rows and `tail()` to obtain the last few rows of our `samples` data frame.
...
We notice that the sample proportions are centred around the population proportion value, `r round(mean(sample_estimates$sample_proportion), 2)`! In general, the mean of the distribution of $\hat{p}$ should be equal to $p$, which is good because the sample proportion is neither an overestimate nor an underestimate of the population proportion.

So what can we learn from this sampling distribution? This distribution tells us what we might expect from proportions from samples of size $40$ when our population proportion is 0.63. In practice, we usually don't actually know the proportion of our population, but if we can use what we know about what the sampling distribution looks like when we do then we can use it to make inferences about our population when only having a single sample. 
...
Timbits. As mentioned in the introduction to this chapter, there are many choices of population parameter for each type of observed variable. What if we wanted to infer something about a population of *quantitative* variables instead? For instance, a traveller visiting Vancouver, BC may wish to know about the prices of staying somewhere using Airbnb, an online marketplace for arranging places to stay. Particularly, they might be interested in estimating the population mean price per night of Airbnb listings in Vancouver, BC. In this section, we will study the case where we are interested in the population mean of a quantitative variable.
...
```{r 11-example-means3, echo = TRUE, message = FALSE, warning = FALSE, fig.cap = "Distribution of price per night ($) for sample of 40 Airbnb listings", out.width = "50%"}
sample_1 <- airbnb %>% 
    rep_sample_n(40)
head(sample_1)
...
```{r 11-example-means4, echo = TRUE, message = FALSE, warning = FALSE, fig.cap= "Sampling distribution of the sample means for sample size of 40"}
...
grid.arrange(population_distribution + ggtitle("Population distribution"), 
             ...
             sampling_distribution_40 + ggtitle("Sampling distribution of the sample mean"), nrow = 3)
```
...
sampling_distribution_20 <-  ggplot(sample_estimates_20, aes(x = sample_mean)) +
    ...
    xlab("Sample mean price per night($)") +
    ggtitle("n = 20") 
sampling_distribution_20  <- sampling_distribution_20  + xlim(min_x(sampling_distribution_20), max_x(sampling_distribution_20))
...
boot3_dist <- ggplot(boot3, aes(price)) + 
    geom_histogram(fill="dodgerblue3", color="lightgrey") + xlab("Price") +
  theme(axis.ticks.x = element_blank(),
        axis.text.x = element_blank(), 
        axis.title.y =element_blank(), 
        axis.ticks.y = element_blank(),

...
to specify the number of samples to take, here 15,000.
...
We notice that the sample proportions are centred around the population proportion value, `r round(mean(sample_estimates$sample_proportion), 2)`! In general, if we take repeated random samples from the population, we expect all the sample proportions to average out to the population proportion. This means that the sample proportion is neither an overestimate or an underestimate of the population proportion. This distribution tells us what we might expect from proportions from samples of size $40$ when our population proportion is 0.63. If we can somehow learn what sampling distributions look like, we can use this knowledge to make statistical inference about our population parameter when we are in the more common, and realistic scenario of only having a single sample and not knowing the population proportion. 

...
In this section, we will discuss **interval estimation** and construct **confidence intervals** 
...
a *confidence interval* for the mean, a range of plausible values for the
...
interval using the bootstrap distribution we found earlier.
...
We're 95\% "confident" that the true population mean price per night of Airbnb
listings in Vancouver is between \$`r round(bounds[1],2) ` and \$`r round(bounds[2],2)`. Suppose 
we repeated this process of sampling and constructing confidence intervals more times with more samples. We'd expect
95\% of intervals would contain the true population parameter. 

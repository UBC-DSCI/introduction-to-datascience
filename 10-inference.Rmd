...
sampling_dist <- ggplot(sample_estimates, aes(x = sample_mean),  
                          xlim(90,250), ylim=c(0, max_count(boot_est_dist))) +
  geom_histogram(fill = "dodgerblue3", color = "lightgrey") +
  xlab("Sample mean price per night ($)")  

annotated_sampling_dist <- sampling_dist+  
  annotate("text",
           x = max_x(boot_est_dist), 
           y = 2000,
           label = paste("mean = ", round(mean(sample_estimates$sample_mean), 2))) +
    geom_vline(xintercept = mean(sample_estimates$sample_mean), col = "red") 
annotated_boot_est_dist <- boot_est_dist +   xlim(c(90,250)) + 
  annotate("text",
    x = max_x(boot_est_dist), y = 2000, vjust = 1, hjust = 1, 

...
Here, the population consists of all one-bedroom apartment rental offerings in Vancouver, and the population
parameter is the *average price-per-night*. But even within this one example, we could also be interested
...
<!--- CC0 https://commons.wikimedia.org/wiki/File:Timbits2.jpg --->
...
The population mean is \$`r round(population_parameters$pop_mean,2)` and the
population standard deviation is \$`r round(population_parameters$pop_sd, 2)`.

Suppose we take a sample of 20 observations from our population. Below we 
...
are **point estimates** for the mean and standard deviation of the full population.
...
was \$`r round(population_parameters$pop_mean,2)` and the population standard deviation
was \$`r round(population_parameters$pop_sd,2)`. We see that our point
estimates for the mean and standard deviation 
are \$`r round(estimates$sample_mean, 2)` and  \$`r round(estimates$sample_sd,2)`,
...
about `r round(100*abs(estimates$sample_sd - population_parameters$pop_sd)/population_parameters$pop_sd, 1)`% off.
...
we can take many samples and plot the **sampling distribution** of the point estimates to get a sense
...
sample_estimates_500 <- rep_sample_n(airbnb, size = 500, reps = 1500) %>% 
    group_by(replicate) %>% 
    summarise(sample_mean = mean(price))
...
```{r 10-example-means7,  echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Comparision of sampling distributions"}
...
In this section, we will discuss **interval estimation**
using just a single sample from a population.
...
```{r 10-example-bootstrapping0, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Comparision of samples of different sizes from the population"}
...
```{r 10-bootstrapping1, echo = TRUE, message = FALSE, warning = FALSE, fig.cap = "Histogram of price per night ($) for one sample of size 40"}
one_sample <- airbnb %>% 
    rep_sample_n(40) %>% 
...
sampling_dist <- sampling_dist +
  annotate("text", x = max_x(sampling_dist), max_count(sampling_dist), hjust = 1, vjust =3,
           label = paste("mean = ", round(mean(sample_estimates$sample_mean), 1))) + 
  annotate("text", x = max_x(sampling_dist), max_count(sampling_dist), hjust = 1, vjust =5,
           label = paste("sd = ", round(sd(sample_estimates$sample_mean), 1))) + 
  geom_vline(xintercept = mean(sample_estimates$sample_mean), col = "red")

annotated_boot_est_dist <- boot_est_dist + 
  annotate("text", x = max_x(boot_est_dist), max_count(boot_est_dist), hjust = 1, vjust =0,
           label = paste("mean = ", round(mean(boot1500_means$mean), 1))) + 
    annotate("text", x = max_x(boot_est_dist), max_count(boot_est_dist), hjust = 1, vjust =2,
...
Now that we have constructed our bootstrap distribution let's use it to find a range of plausible values for the population mean. We will find the range of values that cover the middle 95\% of the bootstrap distribution. Note that there's nothing particularly special about 95\%, we could have used other levels, such as 90\% or 99\%. The level you choose will depend on your application, which is beyond the scope of this textbook.

To calculate our interval estimate, we will do the following:

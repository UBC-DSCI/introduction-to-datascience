...
grid.arrange(population_distribution + 
               geom_vline(xintercept = mean(airbnb$price, na.rm = T), col = "red") +  
    ggtitle("Population distribution")  + 
               annotate("text", 500, 800, 
                        label = paste("mean = ", round(mean(airbnb$mean), 1))), 
             sampling_distribution_20 + 
               geom_vline(xintercept = mean(sample_estimates$sample_mean), col = "red") +
               xlim(80, 250) + 
               ggtitle("n = 20") + 
               annotate("text", 210, 150, 
             ...
             sampling_distribution_50 + 
               ...
               xlim(80, 250) + 
               annotate("text", 210, 250, 
                        label = paste("mean = ", 
                                      round(mean(sample_estimates_50$sample_mean), 1))),
             sampling_distribution_100 + 
               geom_vline(xintercept = mean(sample_estimates_100$sample_mean), col = "red")+
               xlim(80, 250) + 
               annotate("text", 210, 250, 
                        label = paste("mean = ", 
                                      round(mean(sample_estimates_100$sample_mean), 1))),
             nrow = 2, ncol = 2) 
```
...
sampling_dist <-  ggplot(sample_estimates, aes(x = sample_mean)) +
    ...
    annotate("text", 200, 100, label = paste("mean = ", round(mean(sample_estimates$sample_mean), 1)))+ 
  geom_vline(xintercept = mean(sample_estimates$sample_mean), col = "red")
boot_est_dist <- boot_est_dist + 
    annotate("text", x = 200, y = 100, label = paste("mean = ", round(mean(boot1000_means$mean), 1))) + 

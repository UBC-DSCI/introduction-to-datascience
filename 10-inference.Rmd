...
So in this chapter, we will simulate a synthetic box of 10,000 Timbits with two flavours&mdash;old-fashioned 
and chocolate&mdash;as our population, and use this to illustrate
inferential concepts. Below we have a `tibble()` called `virtual_box` with a subject ID and Timbit flavour as our columns. We have also loaded our necessary packages: `tidyverse` and the `infer` package, which we will need to perform sampling later in the chapter.  
...
choc_sample_1 <- summarize(samples_1, n = sum(flavour == "chocolate"),

...
the previous section. However, the bootstrap distribution is centred at \$`r round(mean(boot15000_means$mean), 2)`, which 
...
original sample's mean value (unlike the sampling distribution of the sample mean which is centred at the population parameter value). The second important point is that the shape and spread of the true sampling distribution
and the bootstrap distribution is similar; the bootstrap distribution lets us get a sense for the variability of the point estimate.
...
# dots 
mygb(x=c(0.35,0.35,0.45,0.45), y=c(0.4,0.4,0.01,0.01))
  grid.text("many more resamples", x = unit(0.45, "npc"), 
            y = unit(0.01, "npc"),gp = gpar(fontsize=10))
  
mygb(x=c(0.35,0.35,0.7,0.7), y=c(0.4,0.4,0.01,0.01))
  grid.text("many more means", x = unit(0.45, "npc"), 
            y = unit(0.01, "npc"),gp = gpar(fontsize=10))
...
For more about statistical inference and bootstrapping, refer to Chapters 7 - 8 of [Modern Dive](https://moderndive.com/) Statistical 
Inference via Data Science by Chester Ismay and Albert Y. Kim or Chapters 4 - 7 of [OpenIntro Statistics - Fourth Edition](https://www.openintro.org/) by David M. Diez, Christopher D. Barr and Mine Cetinkaya-Rundel.

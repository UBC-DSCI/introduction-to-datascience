...
Now we will generate a single bootstrap sample in R using the sample that we just took. The process for doing this is as follows: 
1. Randomly select an observation from the data of size $n$
2. Replace that observation
3. Randomly select an observation again from the sample data until you have $n$ observations 
4. Calculate the statistic on the sample of size $n$
5. Repeat this procedure $N$ number of times

We will use the `rep_sample_n` function as we did when we were creating our sampling distribution. Since we want to sample with replacement we change the argument for `replace` from its default value of `FALSE` to `TRUE`.
...
boot_est_dist <- boot_est_dist + 
    annotate("text", x = 220, y = 100, label = paste("mean = ", round(mean(boot1000_means$mean), 1))) + 

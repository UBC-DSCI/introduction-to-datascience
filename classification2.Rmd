...
We can use `group_by` and `summarize` to find the proportion of malignant and benign classes 
in `cancer_train`.
...
train_prop <- cancer_train |> 
  ...
  summarize(proportion = n()/nrow(cancer_train))
```
We see `r round(filter(train_prop, Class == "B")$proportion, 3)*100`% of the training
data are benign and `r round(filter(train_prop, Class == "M")$proportion, 3)*100`% 
are malignant indicating that our class proportions were roughly preserved when we split the data.

### Preprocess the data

As we mentioned last chapter, $K$-nearest neighbors is sensitive to the scale of the predictors,
...
as maligant, and `r confu22` were correctly predicted as benign. Therefore the classifier labelled 
...
```{r 06-lots-of-ks, message = FALSE, fig.height = 4, fig.width = 5, fig.cap="Plot of accuracy estimate versus number of neighbours for many K values"}
set.seed(1)
...
a balance between the two. You can see these two effects in Figure \@ref(fig:06-decision-grid-K),
...
can tune the classifier (e.g., select the number of neighbours $K$ in $K$-nn)
...
```{r 06-fwdsel-3, echo = FALSE, warning = FALSE, fig.retina = 2, out.width = "600", fig.cap = "Estimated accuracy versus the number of predictors for the sequence of models built using forward selection"}
